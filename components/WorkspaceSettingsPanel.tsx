...
function Toggle({ checked, onChange, title, description }: { checked: boolean; onChange: (checked: boolean) => void; title: string; description: string }) {
  return <label className="settings-toggle"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span className="settings-switch" aria-hidden="true" /><span className="settings-toggle-copy"><strong>{title}</strong><small>{description}</small></span></label>;
}
  ...
  return <section className="settings-workspace operations-workspace">
    ...
    <div className="settings-grid">
      ...
      <section className="panel settings-group settings-privacy">
        <header><p className="eyebrow">Privacy and records</p><h3>Local operational history</h3></header>
        <div className="settings-privacy-notice"><strong>Metadata only</strong><p>Case and filing history stores client name, status, round and delivery dates locally. Raw source TXT and document file contents are not included in records export.</p></div>
        <button type="button" className="settings-record-action" onClick={onExportRecords}><div><strong>Export records</strong><small>Download local case and filing metadata as JSON.</small></div><span>Export</span></button>
        {!confirmClear ? <button type="button" className="settings-record-action danger" onClick={() => setConfirmClear(true)}><div><strong>Clear operational history</strong><small>Remove local case and filing tracker records.</small></div><span>Clear</span></button> : <div className="settings-clear-confirm"><strong>Clear all local case and filing records?</strong><p>This does not remove configured templates.</p><div><button type="button" className="secondary-button" onClick={() => setConfirmClear(false)}>Cancel</button><button type="button" className="danger-button" onClick={() => { onClearRecords(); setConfirmClear(false); }}>Clear Records</button></div></div>}
      </section>
    </div>
  </section>;
}
