...
function sequence(type: LetterType, evidence: boolean) { return type === 'LATE_PAYMENT' ? ['01 Late Payment Letter', ...(evidence ? ['02 Supporting Documents'] : [])] : ['01 Dispute Letter', ...(evidence ? ['02 Supporting Documents'] : []), '03 FCRA', '04 Affidavit', '05 Attachment', '06 FTC']; }
...
export default function LetterGeneratorWorkspace() {
  ...
  const [status, setStatus] = useState('Configure the packet templates, then load a client source file.');
  ...
  const neededLetterTypes = Array.from(new Set(routes.map((route) => route.type)));
  const missingLetters = neededLetterTypes.filter((type) => !currentReferences.find((item) => item.type === type)?.file);
  ...
  async function createWorkingZip(docs: ReviewOutput[], notices: string[], date: string) { const zip = new JSZip(); docs.forEach((doc) => zip.file(doc.path, doc.blob)); zip.file('Generation Manifest.txt', ['WORKING DOCUMENTS - REVIEW BEFORE FINALIZATION', `Client: ${parsed.name}`, `Round: ${round}`, `Date: ${date}`, '', 'Dispute PDF order:', '01 Dispute Letter', '02 Supporting Documents', '03 FCRA', '04 Affidavit', '05 Attachment', '06 FTC', '', 'Late Payment PDF order:', '01 Late Payment Letter', '02 Supporting Documents', '', 'Editable documents:', ...docs.map((doc) => `- ${doc.path}`), ...(notices.length ? ['', 'Notices:', ...notices.map((notice) => `- ${notice}`)] : [])].join('\n')); return zip.generateAsync({ type: 'blob' }); }
  async function generateReviewDocuments() {
    ...
    setBusy(true); const date = dateInEasternTime(); const docs: ReviewOutput[] = []; const notices: string[] = []; let evidencePages: PacketPage[] = [];
    try { evidencePages = evidenceKey ? await getSupportingPages(evidenceKey) : []; } catch { notices.push('Supporting Documents page could not be prepared.'); }
    if (routes.some((route) => route.type === 'DISPUTE')) (['FCRA', 'AFFIDAVIT', 'ATTACHMENT', 'FTC'] as ExhibitKind[]).forEach((kind) => { if (!templates[kind]) notices.push(`${exhibitTitles[kind]} is missing for final Dispute PDF assembly.`); });
    for (const route of routes) {
      ...
      try {
        let letter = await createLetter(route, template, date); if (evidencePages.length) letter = await appendSupportingPages(letter, evidencePages);
        const prefix = `${clean(parsed.name)} ${route.bureau}`;
        docs.push({ id: `${route.type}-${route.bureau}-LETTER`, path: `Editable Documents/${prefix} ${typeLabel[route.type]}.docx`, type: route.type, role: 'LETTER', sequence: 1, bureau: route.bureau, count: route.items.length, detail: `${route.reason}${evidencePages.length ? ' - Supporting page included' : ''}`, blob: letter, packetSteps: sequence(route.type, Boolean(evidencePages.length)) });
        if (route.type === 'DISPUTE') for (const item of [{ kind: 'AFFIDAVIT' as const, role: 'AFFIDAVIT' as const, number: 4 }, { kind: 'FTC' as const, role: 'FTC' as const, number: 6 }]) {
          if (!templates[item.kind]) continue; const mapped = await createMappedDoc(item.kind, route.bureau, date);
          if (mapped) docs.push({ id: `${route.bureau}-${item.kind}`, path: `Editable Documents/${prefix} ${String(item.number).padStart(2, '0')} ${exhibitTitles[item.kind]}.docx`, type: 'DISPUTE', role: item.role, sequence: item.number, bureau: route.bureau, count: parsed.dispute[route.bureau].length, detail: 'Mapped DOCX template - edit and inspect page breaks before finalization', blob: mapped });
        }
      } catch (error) { notices.push(`${typeLabel[route.type]} / ${route.bureau}: ${error instanceof Error ? error.message : 'Generation failed.'}`); }
    }
    const zip = await createWorkingZip(docs, notices, date); setReviewDocs(docs); setWarnings(notices); setWorkingZip({ name: `${fileBase(parsed.name)}_${fileBase(round)}_WORKING_DOCUMENTS.zip`, blob: zip }); setDocumentDate(date); setFinalPackets([]); setFinalZip(null); setBusy(false); setPanel('Outputs'); setStatus(`${docs.length} editable document(s) prepared. Review them before PDF finalization.`);
  }
  async function saveEdited(output: ReviewOutput, file: File) { const docs = reviewDocs.map((item) => item.path === output.path ? { ...item, blob: file, detail: 'Edited and saved for finalization' } : item); const zip = await createWorkingZip(docs, warnings, documentDate || dateInEasternTime()); setReviewDocs(docs); setWorkingZip({ name: workingZip?.name || 'WORKING_DOCUMENTS.zip', blob: zip }); setFinalPackets([]); setFinalZip(null); setStatus('Edit saved. Re-run PDF finalization after completing document review.'); }
  async function finalizePdfPackets() {
    setFinalizing(true); const packets: FinalPdfPacket[] = []; const notices: string[] = [];
    if (!evidence.supporting.length) notices.push('Supporting Documents must be uploaded before final PDF packet assembly.');
    for (const route of routes) {
      const letter = reviewDocs.find((doc) => doc.type === route.type && doc.bureau === route.bureau && doc.role === 'LETTER');
      if (!letter) { notices.push(`${route.bureau}: generated letter is unavailable.`); continue; }
      const parts: PdfPacketPart[] = [{ label: typeLabel[route.type], kind: 'DOCX', blob: letter.blob }];
      if (route.type === 'DISPUTE') {
        const fcra = await readTemplateExhibit(round, 'FCRA'); const attachment = await readTemplateExhibit(round, 'ATTACHMENT'); const affidavit = reviewDocs.find((doc) => doc.bureau === route.bureau && doc.role === 'AFFIDAVIT'); const ftc = reviewDocs.find((doc) => doc.bureau === route.bureau && doc.role === 'FTC');
        if (!fcra || !attachment || !affidavit || !ftc) { notices.push(`${route.bureau} Dispute PDF is incomplete: FCRA, Affidavit, Attachment and FTC are required.`); continue; }
      ...
      try { packets.push({ path: `Final PDF Packets/${clean(parsed.name)} ${route.bureau} ${route.type === 'DISPUTE' ? 'DISPUTE' : 'LATE PAYMENT'} PACKET.pdf`, type: route.type, bureau: route.bureau, sequence: sequence(route.type, Boolean(evidence.supporting.length)), blob: await assembleFinalPdf(parts) }); } catch (error) { notices.push(`${route.bureau}: ${error instanceof Error ? error.message : 'PDF assembly failed.'}`); }
    }
    ...
    setFinalPackets(packets); setFinalZip(finalOutput); setWarnings([...warnings, ...notices]); setFinalizing(false); setStatus(packets.length ? `${packets.length} final PDF packet(s) ready for review.` : 'Final PDF assembly requires attention.');
  }
  ...
  function generateView() { return <div className="generation-workspace"><section className="panel generation-overview"><div><p className="eyebrow">Packet plan</p><h2>Prepare editable review documents</h2><p>Letters, Affidavit and FTC remain editable until PDF finalization.</p></div><div className="generation-summary"><div><strong>{routes.length}</strong><span>Letters</span></div><div><strong>{evidence.supporting.length ? 'Yes' : 'No'}</strong><span>Support</span></div><div><strong>{configuredExhibits(templates).length}/4</strong><span>Dispute nodes</span></div></div></section><section className="panel route-production"><div className="production-rows">{routes.map((route) => <div className="production-row" key={`${route.bureau}-${route.type}`}><div><strong>{route.bureau} - {typeLabel[route.type]}</strong><small>{route.reason}</small><div className="assembly-chips">{sequence(route.type, Boolean(evidence.supporting.length)).map((entry) => <span className="assembly-chip" key={entry}>{entry}</span>)}</div></div><Pill tone="success">Prepare</Pill></div>)}</div>{missingLetters.length > 0 && <div className="alert error">Missing letter DOCX: {missingLetters.map((type) => typeLabel[type]).join(', ')}.</div>}<button className="action-button generate-primary" disabled={busy || (strict && missingLetters.length > 0)} onClick={() => void generateReviewDocuments()}>{busy ? 'Preparing documents...' : 'Generate Editable Review Documents'}</button></section></div>; }
  function outputsView() { return <OutputReviewWorkspace round={round} outputs={reviewDocs} zipName={workingZip?.name} warnings={warnings} finalPackets={finalPackets} finalizing={finalizing} finalZipName={finalZip?.name} onZip={() => workingZip && deliver(workingZip.name, workingZip.blob)} onFinalZip={() => finalZip && deliver(finalZip.name, finalZip.blob)} onFinalize={finalizePdfPackets} onPdfDownload={(packet) => deliver(packet.path.split('/').pop() || 'packet.pdf', packet.blob)} onReplace={saveEdited} />; }
