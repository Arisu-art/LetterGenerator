...
import { resolveTemplateBindingsAgainstMatrix, sourceItems, sourceText } from '../lib/packet-template-bindings';
import { matrixIssueSummary, matrixTraceSummary, type MatrixTrace } from '../lib/packet-transformation-matrix';
...
export default function LetterGeneratorWorkspace() {
  ...
  const [matrixTrace, setMatrixTrace] = useState<string[]>([]);
  ...
  function clearOutputs() { setReviewDocs([]); setWarnings([]); setMatrixTrace([]); setWorkingZip(null); setFinalPackets([]); setFinalZip(null); setDocumentDate(''); }
  ...
  async function createLetter(route: LetterRoute, template: File, date: string): Promise<{ blob: Blob; trace: MatrixTrace[] }> {
    const recipient = bureauInfo[route.bureau];
    const bindings = resolveTemplateBindingsAgainstMatrix({ parsed, route, documentDate: date, bureauName: recipient.name, bureauAddressLines: recipient.address.split('\n') });
    if (!bindings.ok) throw new Error(failMessage(matrixIssueSummary(bindings)));
    const value = bindings.value;
    const identity = { consumerName: sourceText(value.consumer_name), addressLines: splitLines(sourceText(value.address)), dob: sourceText(value.dob), ssn: sourceText(value.ssn), letterDate: sourceText(value.date), bureauName: sourceText(value.bureau_name), bureauAddressLines: splitLines(sourceText(value.bureau_address)) };
    const blob = route.type === 'DISPUTE'
      ? await renderReferenceDisputeDocx(template, { ...identity, disputeItems: sourceItems(value.accounts).map((item) => item.displayText), hardInquiryItems: sourceItems(value.hard_inquiries).map((item) => item.displayText) })
      : await renderLatePaymentReference(template, { ...identity, latePaymentItems: sourceItems(value.late_payment_items).map((item) => item.displayText) });
    return { blob, trace: bindings.trace };
  }
  ...
  async function createWorkingZip(docs: ReviewOutput[], notices: string[], date: string, traceLines = matrixTrace) { const zip = new JSZip(); docs.forEach((doc) => zip.file(doc.path, doc.blob)); zip.file('Generation Manifest.txt', ['WORKING DOCUMENTS - REVIEW BEFORE FINALIZATION', `Client: ${parsed.name}`, `Round: ${round}`, `Date: ${date}`, '', 'Matrix trace:', ...(traceLines.length ? traceLines : ['No matrix trace recorded.']), '', 'Review stage:', 'Edit generated DOCX files only: letters and Affidavit.', 'Supporting Documents and static PDFs are merged into final PDF packets.', '', 'Dispute final PDF order:', ...sequence('DISPUTE'), '', 'Late Payment final PDF order:', ...sequence('LATE_PAYMENT'), '', 'Editable documents:', ...docs.map((doc) => `- ${doc.path}`), ...(notices.length ? ['', 'Needs attention:', ...notices.map((notice) => `- ${notice}`)] : [])].join('\n')); return zip.generateAsync({ type: 'blob' }); }
  ...
  async function generateReviewDocuments() {
    ...
    for (const route of routes) {
      ...
      try {
        const rendered = await createLetter(route, template, date);
        traceLines.push(...matrixTraceSummary({ ok: true, value: null, trace: rendered.trace }));
        const prefix = `${clean(parsed.name)} ${route.bureau}`;
        docs.push({ id: `${route.type}-${route.bureau}-LETTER`, path: `Editable Documents/${prefix} ${typeLabel[route.type]}.docx`, type: route.type, role: 'LETTER', sequence: 1, bureau: route.bureau, count: route.items.length, detail: `${route.reason} · Matrix-bound packet output`, blob: rendered.blob, packetSteps: sequence(route.type) });
    ...
    const zip = await createWorkingZip(docs, notices, date, traceLines);
    setReviewDocs(docs); setWarnings(notices); setMatrixTrace(traceLines); setWorkingZip({ name: `${fileBase(parsed.name)}_${fileBase(round)}_WORKING_DOCUMENTS.zip`, blob: zip }); setDocumentDate(date); setFinalPackets([]); setFinalZip(null); setBusy(false); setPanel('Outputs'); setStatus(`${docs.length} editable document(s) prepared through matrix-bound rendering.`);
  }
  async function saveEdited(output: ReviewOutput, file: File) { const docs = reviewDocs.map((item) => item.path === output.path ? { ...item, blob: file, detail: 'Edited and saved for finalization' } : item); const zip = await createWorkingZip(docs, warnings, documentDate || dateInEasternTime(), matrixTrace); setReviewDocs(docs); setWorkingZip({ name: workingZip?.name || 'WORKING_DOCUMENTS.zip', blob: zip }); setFinalPackets([]); setFinalZip(null); setStatus('Edit saved. Use Complete Packet Preview to inspect the merged page order before finalizing.'); }
  ...
  async function downloadAllPacketPdfs() { setFinalizing(true); const { packets, notices } = await buildFinalPdfPackets(); if (!packets.length) { setWarnings((previous) => uniqueNotices(previous, notices)); setFinalizing(false); setStatus(failMessage(notices)); return; } const zip = new JSZip(); packets.forEach((packet) => zip.file(packet.path.split('/').pop() || 'PACKET.pdf', packet.blob)); zip.file('Final Packet Manifest.txt', packets.flatMap((packet) => [packet.path.split('/').pop() || packet.path, ...packet.sequence.map((entry) => `  ${entry}`), '']).join('\n')); const blob = await zip.generateAsync({ type: 'blob' }); const name = `${fileBase(parsed.name)}_${fileBase(round)}_ALL_BUREAU_PDF_PACKETS.zip`; setFinalPackets(packets); setFinalZip({ name, blob }); setWarnings((previous) => uniqueNotices(previous, notices)); setFinalizing(false); deliver(name, blob); setStatus(`${packets.length} bureau merged PDF packet(s) downloaded in one ZIP.`); }
