...
export default function LetterGeneratorWorkspace() {
  ...
  async function downloadAllPacketPdfs() {
    ...
    if (!packets.length) { setWarnings((previous) => uniqueNotices(previous, notices)); setFinalizing(false); setStatus('No final PDF packets were available to download.'); return; }
    const zip = new JSZip();
