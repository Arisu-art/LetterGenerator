...
export default function LetterGeneratorWorkspace() {
  ...
  async function createWorkingZip(docs: ReviewOutput[], notices: string[], date: string) { const zip = new JSZip(); docs.forEach((doc) => zip.file(doc.path, doc.blob)); zip.file('Generation Manifest.txt', ['WORKING DOCUMENTS - REVIEW BEFORE FINALIZATION', `Client: ${parsed.name}`, `Round: ${round}`, `Date: ${date}`, '', 'Review stage:', 'Edit generated DOCX files only: letters, Affidavit and FTC.', 'Supporting Documents and static PDFs are inserted in final assembly order after editing.', '', 'Dispute final PDF order:', ...sequence('DISPUTE'), '', 'Late Payment final PDF order:', ...sequence('LATE_PAYMENT'), '', 'Editable documents:', ...docs.map((doc) => `- ${doc.path}`), ...(notices.length ? ['', 'Needs attention:', ...notices.map((notice) => `- ${notice}`)] : [])].join('\n')); return zip.generateAsync({ type: 'blob' }); }
  ...
  async function generateReviewDocuments() {
    ...
    if (!evidence.supporting.length) notices.push('Supporting Documents are required before final PDF assembly. Upload them in Source Data.');
    missingDisputeNodes.forEach((kind) => notices.push(`${exhibitTitles[kind]} is missing for final Dispute PDF assembly.`));
    for (const route of routes) {
      ...
      try {
        ...
        docs.push({ id: `${route.type}-${route.bureau}-LETTER`, path: `Editable Documents/${prefix} ${typeLabel[route.type]}.docx`, type: route.type, role: 'LETTER', sequence: 1, bureau: route.bureau, count: route.items.length, detail: `${route.reason} · Supporting page is inserted during PDF finalization`, blob: letter, packetSteps: sequence(route.type) });
    ...
    setReviewDocs(docs); setWarnings(notices); setWorkingZip({ name: `${fileBase(parsed.name)}_${fileBase(round)}_WORKING_DOCUMENTS.zip`, blob: zip }); setDocumentDate(date); setFinalPackets([]); setFinalZip(null); setBusy(false); setPanel('Outputs'); setStatus(`${docs.length} editable document(s) prepared. Review page boundaries, then finalize PDF packets.`);
  }
  async function saveEdited(output: ReviewOutput, file: File) { const docs = reviewDocs.map((item) => item.path === output.path ? { ...item, blob: file, detail: 'Edited and saved for finalization' } : item); const zip = await createWorkingZip(docs, warnings, documentDate || dateInEasternTime()); setReviewDocs(docs); setWorkingZip({ name: workingZip?.name || 'WORKING_DOCUMENTS.zip', blob: zip }); setFinalPackets([]); setFinalZip(null); setStatus('Edit saved. Review the page guides again, then finalize PDF packets.'); }
  async function finalizePdfPackets() {
    ...
    if (!supportingPdf) {
      notices.push('Supporting Documents page is required before final PDF packet assembly. Upload client proof images in Source Data.');
      setFinalPackets([]); setFinalZip(null); setWarnings([...warnings, ...notices]); setFinalizing(false); setStatus('Final PDF assembly stopped: add Supporting Documents in Source Data.'); return;
    }
    for (const route of routes) {
      ...
      const parts: PdfPacketPart[] = [{ label: typeLabel[route.type], kind: 'DOCX', blob: letter.blob }, { label: 'Supporting Documents', kind: 'PDF', blob: supportingPdf }];
      if (route.type === 'DISPUTE') {
        const fcra = await readTemplateExhibit(round, 'FCRA');
        const attachment = await readTemplateExhibit(round, 'ATTACHMENT');
        const affidavit = reviewDocs.find((doc) => doc.bureau === route.bureau && doc.role === 'AFFIDAVIT');
        const ftc = reviewDocs.find((doc) => doc.bureau === route.bureau && doc.role === 'FTC');
      ...
      try { packets.push({ path: `Final PDF Packets/${clean(parsed.name)} ${route.bureau} ${route.type === 'DISPUTE' ? 'DISPUTE' : 'LATE PAYMENT'} PACKET.pdf`, type: route.type, bureau: route.bureau, sequence: sequence(route.type), blob: await assembleFinalPdf(parts) }); } catch (error) { notices.push(`${route.bureau}: ${error instanceof Error ? error.message : 'PDF assembly failed.'}`); }
    }
    ...
    setFinalPackets(packets); setFinalZip(finalOutput); setWarnings([...warnings, ...notices]); setFinalizing(false); setStatus(packets.length ? `${packets.length} final PDF packet(s) ready for page-order review.` : 'Final PDF assembly requires attention.');
  }
  ...
  function generateView() { return <div className="generation-workspace"><section className="panel generation-overview"><div><p className="eyebrow">Packet plan</p><h2>Prepare editable review documents</h2><p>Letters, Affidavit and FTC remain editable. Supporting Documents and PDF inserts are added only during final PDF assembly.</p></div><div className="generation-summary"><div><strong>{routes.length}</strong><span>Letters</span></div><div><strong>{evidence.supporting.length ? 'Yes' : 'No'}</strong><span>Support</span></div><div><strong>{configuredExhibits(templates).length}/4</strong><span>Dispute nodes</span></div></div></section><section className="panel route-production"><div className="production-rows">{routes.map((route) => <div className="production-row" key={`${route.bureau}-${route.type}`}><div><strong>{route.bureau} - {typeLabel[route.type]}</strong><small>{route.reason}</small><div className="assembly-chips">{sequence(route.type).map((entry) => <span className="assembly-chip" key={entry}>{entry}</span>)}</div></div><Pill tone="success">Prepare</Pill></div>)}</div>{missingLetters.length > 0 && <div className="alert error">Missing letter DOCX: {missingLetters.map((type) => typeLabel[type]).join(', ')}.</div>}{!evidence.supporting.length && <div className="alert error">Supporting Documents are required for final PDF packets. Upload them in Source Data before finalization.</div>}<button className="action-button generate-primary" disabled={busy || (strict && missingLetters.length > 0)} onClick={() => void generateReviewDocuments()}>{busy ? 'Preparing documents...' : 'Generate Editable Review Documents'}</button></section></div>; }
