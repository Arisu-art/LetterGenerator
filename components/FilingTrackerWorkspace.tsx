...
export default function FilingTrackerWorkspace({ records, onReturnToOutputs, onStartCase, onMarkSent }: Props) {
  ...
  return <section className="filing-tracker-workspace operations-workspace">
    <section className="panel operations-command">
      <div className="operations-heading"><p className="eyebrow">Delivery operations</p><h2>Filing Tracker</h2><p>Track generated bureau packets from final PDF creation through sending.</p></div>
      <div className="operations-actions"><button type="button" className="secondary-button" onClick={onReturnToOutputs}>Open Outputs</button><button type="button" className="action-button" onClick={onStartCase}>New Case</button></div>
    </section>
    <div className="operations-metrics" aria-label="Filing status summary">
      <article><small>Packet records</small><strong>{records.length}</strong></article>
    ...
    <section className="panel operations-table-surface">
      <header className="operations-section-head"><div><h3>Bureau delivery queue</h3><p>Mark a packet sent after delivery is completed outside the application.</p></div><span className="operations-count">{records.length} filing{records.length === 1 ? '' : 's'}</span></header>
      {records.length === 0 ? <div className="operations-empty"><strong>No final packets tracked</strong><p>Create final PDF packets in Outputs to begin the delivery record.</p><button type="button" className="action-button" onClick={onReturnToOutputs}>Go to Outputs</button></div> : <div className="filing-records" role="list">{records.map((record) => <article key={record.id} className="filing-record" role="listitem">
