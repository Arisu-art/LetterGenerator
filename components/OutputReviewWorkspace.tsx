...
export default function OutputReviewWorkspace({ round, outputs, expectedRoutes, zipName, warnings, evidenceKey = '', evidence, onEvidenceChanged, onMessage, onZip, onReplace }: Props) {
  ...
  return <section className="outputs-workspace guided-output-workspace progressive-output-workspace"><section className="panel output-stage output-review-stage shared-stage-surface">
    <header className="output-stage-header output-progressive-command"><div className="output-stage-heading"><p className="eyebrow">Review and delivery</p><h2>Complete ordered package</h2><p>Each bureau packet below is the authoritative filing-order view. Open its editor to modify the letter or shared affidavit, then download the complete ZIP.</p></div><span className={`output-count-pill ${coverage.complete ? '' : 'blocked'}`}>{coverage.generated}/{coverage.expected} generated</span></header>
