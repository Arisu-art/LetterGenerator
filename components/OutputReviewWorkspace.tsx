...
function roleLabel(output: ReviewOutput) {
  ...
  return output.type === 'DISPUTE' ? 'Dispute' : 'Late Payment';
}
export default function OutputReviewWorkspace({ round, outputs, zipName, warnings, finalPackets = [], finalizing = false, onZip, onFinalize, onPdfDownload, onReplace }: Props) {
  ...
  return <section className="outputs-workspace">
    <section className="panel package-overview">
      <header className="package-header"><div><p className="eyebrow">Review and finalization</p><h2>{round} document packets</h2><p>Edit generated DOCX content first. Final PDF assembly follows the configured packet order.</p></div><span className="package-count">{outputs.length} DOCX</span></header>
