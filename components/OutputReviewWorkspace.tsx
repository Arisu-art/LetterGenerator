...
export default function OutputReviewWorkspace({ round, outputs, zipName, warnings, finalPackets = [], finalizing = false, finalZipName, evidenceKey = '', evidence, onEvidenceChanged, onMessage, onZip, onFinalZip, onFinalize, onPdfDownload, onReplace }: Props) {
  ...
  useEffect(() => { if (finalPackets.length) setStage('DELIVERY'); }, [finalPackets.length]);
  ...
  return <section className="outputs-workspace guided-output-workspace">
    ...
    {stage === 'REVIEW' && <section className="panel output-stage output-review-stage">
      ...
      <footer className="output-stage-footer"><span>{reviewedCount < packets.length ? 'You may continue and return to edit packets before finalizing.' : 'All bureau packets have been opened for review.'}</span><button className="action-button" disabled={!packets.length} onClick={() => setStage('FINALIZE')}>Continue to Finalize</button></footer>
    </section>}

    {stage === 'FINALIZE' && <section className="panel output-stage output-finalize-stage">
      ...
      <footer className="output-stage-footer"><button className="secondary-button" onClick={() => setStage('REVIEW')}>Back to Review</button></footer>
    </section>}

    {stage === 'DELIVERY' && <section className="panel output-stage output-delivery-stage">
      ...
      <footer className="output-stage-footer"><button className="secondary-button" onClick={() => setStage('REVIEW')}>Return to Review</button></footer>
    </section>}
