...
export default function OutputReviewWorkspace({ round, outputs, zipName, warnings, onZip, onDownload, onReplace, onRemove }: Props) {
  ...
  return <section className="outputs-workspace">
    ...
    {selected && <div className="review-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedPath(null); }}><section className="review-modal" role="dialog" aria-modal="true"><header><div><p className="eyebrow">Document review</p><h2>{selected.path.split('/').pop()}</h2><span>{selected.bureau} · {selected.type === 'DISPUTE' ? 'Dispute' : 'Late Payment'}</span></div><button onClick={() => setSelectedPath(null)} aria-label="Close review">×</button></header><div className="review-toolbar"><button onClick={() => onDownload(selected)}>Download DOCX</button><label>Replace Revised DOCX<input type="file" accept=".docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void onReplace(selected, file); event.target.value = ''; }} /></label><button className="remove" onClick={() => void onRemove(selected)}>Remove</button><p>For full format-preserving editing, download, edit in Word or LibreOffice, then replace the reviewed DOCX here.</p></div><div className="review-canvas">{error && <p className="preview-error">{error}</p>}<div className="docx-preview-host" ref={host} /></div></section></div>}
  </section>;
}
