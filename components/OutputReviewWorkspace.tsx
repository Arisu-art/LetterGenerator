...
export default function OutputReviewWorkspace({ round, outputs, zipName, warnings, finalPackets = [], finalizing = false, finalZipName, evidenceKey = '', evidence, onEvidenceChanged, onMessage, onZip, onFinalZip, onFinalize, onPdfDownload, onReplace }: Props) {
  ...
  return <section className="outputs-workspace">
    <section className="panel package-overview">
      <header className="package-header"><div><p className="eyebrow">Review and delivery</p><h2>{round} bureau packets</h2><p>Open one bureau packet at a time. Its Letter, Affidavit and FTC documents remain inside one standard ordered packet.</p></div><span className="package-count">{packets.length} PACKETS</span></header>
