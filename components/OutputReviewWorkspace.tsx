...
export default function OutputReviewWorkspace({
  ...
  const visibleOutputs = outputs.filter((output) => output.role !== 'FTC');
      ...
      <section className="panel output-stage output-review-stage shared-stage-surface">
        ...
        <section className="complete-package-delivery">
          <div>
            ...
            <p>Download the editable package or the PDF package when available.</p>
          </div>
          ...
          <div className="output-download-actions">
            ...
            <button type="button" className="action-button" disabled={!onPdfDownload} onClick={() => onPdfDownload?.()}>
              Download PDF Package ZIP
            </button>
          </div>
        </section>
      </section>
