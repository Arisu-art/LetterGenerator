...
type Props = {
  ...
  onDownloadAllPacketPdfs?: () => void | Promise<void>;
};
...
export default function OutputReviewWorkspace({ round, outputs, expectedRoutes, zipName, warnings, evidenceKey = '', evidence, onEvidenceChanged, onMessage, onZip, onReplace, onDownloadPacketPdf, onDownloadAllPacketPdfs }: Props) {
  ...
  async function downloadAllPackets() {
    if (!onDownloadAllPacketPdfs) return;
    setBulkDownloading(true);
    try { await onDownloadAllPacketPdfs(); }
  ...
  return <section className="outputs-workspace guided-output-workspace progressive-output-workspace"><section className="panel output-stage output-review-stage shared-stage-surface canonical-output-surface">
    <section className="complete-package-delivery final-pdf-bulk-download"><div><p className="eyebrow">One-click final delivery</p><h3>Download all bureau merged PDFs</h3><p>Creates one ZIP folder containing one ordered merged PDF per available bureau. DOCX files are converted to PDF, static PDFs are merged, and bureau order is preserved.</p></div><button type="button" className="action-button" disabled={!coverage.complete || !onDownloadAllPacketPdfs || bulkDownloading} onClick={() => void downloadAllPackets()}>{bulkDownloading ? 'Building all bureau PDFs...' : `Download ${packets.length || 3} Bureau PDF ZIP`}</button></section>
