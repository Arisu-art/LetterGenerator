...
export default function GuidedSourceDataFlow({ source, originalSource, normalized, verified, parsed, routes, sourceWarnings, evidenceKey, evidence, canGenerate, missingLetters, missingInsertCount, strict, busy, onNormalize, onEditSource, onRestore, onEvidenceChanged, onMessage, onGenerate }: Props) {
  ...
  return <div className="guided-source-workspace progressive-source-workspace">
    {stage === 'SOURCE' && method === 'CHOOSE' && !source && <section className="panel source-progressive-stage source-intake-stage shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
      ...
      <div className="source-method-grid">
        <label className="source-method-card source-method-primary">
          ...
          <p>Import an existing client source file and standardize it immediately.</p>
          <span className="source-method-action">Choose file →</span>
          <input type="file" accept=".txt" onChange={(event) => { void uploadFile(event.target.files?.[0]); event.target.value = ''; }} />
        </label>
        <button type="button" className="source-method-card" onClick={() => setMethod('PASTE')}>
          <span className="source-method-number">02</span>
          <strong>Paste source text</strong>
          <p>Paste data manually or begin from the standard TXT structure.</p>
    ...
    {stage === 'SOURCE' && (method !== 'CHOOSE' || Boolean(source)) && <section className="panel source-progressive-stage source-editor-stage shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
      <SourceStageHeader stage="SOURCE" eyebrow="Step 01 · Source TXT" title="Review source data" description="Standardize and confirm the client record before moving to supporting evidence.">
        {source && <span className={`pill ${verified ? 'success' : 'neutral'}`}>{verified ? 'Normalized' : 'Editing'}</span>}
      </SourceStageHeader>
      <div className="source-editor-layout">
        <aside className="source-editor-tools">
          <div className="source-input-summary"><p className="eyebrow">Input method</p><strong>{method === 'UPLOAD' ? 'TXT upload' : 'Paste or manual entry'}</strong><small>{verified ? 'Ready to lock' : 'Needs standardization'}</small></div>
          <label className="source-tool-upload"><span>Replace with TXT</span><input className="file-input" type="file" accept=".txt" onChange={(event) => { void uploadFile(event.target.files?.[0]); event.target.value = ''; }} /></label>
          {!source && <button type="button" className="secondary-button" onClick={() => onNormalize('NAME:\nADDRESS:\nDOB:\nSSN:\n\nDISPUTE ACCOUNTS\n\nTRANSUNION\n\nEQUIFAX\n\nEXPERIAN\n\nHARD INQUIRIES\n', 'Standard format')}>Use blank format</button>}
          {source && !normalized && <button type="button" className="action-button" onClick={() => onNormalize(source, 'Edited')}>Standardize edits</button>}
          {originalSource && <button type="button" className="secondary-button" onClick={onRestore}>Restore original</button>}
          {!source && <button type="button" className="secondary-button" onClick={() => setMethod('CHOOSE')}>Choose another method</button>}
        ...
        <textarea className="guided-source-text source-focused-text" value={source} onPaste={paste} onChange={(event) => onEditSource(event.target.value)} placeholder="Paste TXT source data here…" />
      </div>
    ...
    {stage === 'EVIDENCE' && <section className="guided-evidence-stage source-progressive-evidence shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
    ...
    {stage === 'GENERATE' && <section className="panel source-progressive-stage routes-stage generation-stage shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
      <SourceStageHeader stage="GENERATE" eyebrow="Step 03 · Review" title="Review and generate" description="Confirm detected bureau routes, then create editable ordered packet documents.">
        <span className="pill neutral">{routes.length} output{routes.length === 1 ? '' : 's'}</span>
      </SourceStageHeader>
