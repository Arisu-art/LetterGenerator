...
export default function GuidedSourceDataFlow({ source, originalSource, normalized, verified, parsed, routes, sourceWarnings, evidenceKey, evidence, canGenerate, missingLetters, missingInsertCount, strict, busy, onNormalize, onEditSource, onRestore, onEvidenceChanged, onMessage, onGenerate }: Props) {
  ...
  return <div className="guided-source-workspace">
    ...
    {stage === 'SOURCE' && <section className="panel guided-source-stage source-stage shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
    ...
    {stage === 'EVIDENCE' && <section className="guided-evidence-stage shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
      <header className="guided-stage-command"><div><p className="eyebrow">Step 02</p><h2>Supporting Documents</h2><p>Add evidence for packet position 02, or continue without evidence.</p></div><div><button className="secondary-button" onClick={() => showStage('SOURCE')}>Back to Source</button><button className="action-button" onClick={confirmEvidence}>{evidence.supporting.length ? 'Confirm Evidence' : 'Skip Evidence'}</button></div></header>
    ...
    {stage === 'GENERATE' && <section className="panel guided-source-stage routes-stage generation-stage shared-stage-surface" style={{ viewTransitionName: 'source-work-stage' }}>
      ...
      <footer className="guided-stage-footer generate-footer"><button className="secondary-button" onClick={() => showStage('EVIDENCE')}>Back to Evidence</button><button className="action-button" disabled={blocked || busy} onClick={() => void onGenerate()}>{busy ? 'Generating package...' : 'Generate Ordered Review Package'}</button></footer>
    </section>}
  </div>;
}
