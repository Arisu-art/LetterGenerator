...
export default function SupportingDocumentsLayoutEditor({ storageKey, assets, toolbarTargetId, onChanged, onMessage }: Props) {
  ...
  function start(event: PointerEvent<HTMLDivElement>, asset: PacketAsset, index: number) { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); setSelectedId(asset.id); setDrag({ id: asset.id, pointerId: event.pointerId, x: event.clientX, y: event.clientY, placement: placement(asset, index, workingAssets.supporting.length), mode: tool }); }
  ...
  function finish(event: PointerEvent<HTMLDivElement>) { if (drag?.pointerId === event.pointerId) { onChanged(latestAssets.current); setDrag(null); onMessage(drag.mode === 'CROP' ? 'Image crop saved for packet position 02.' : 'Evidence layout saved for packet position 02.'); } }
  function resetAll() { const updated = resetSupportingPlacements(storageKey); latestAssets.current = updated; setWorkingAssets(updated); onChanged(updated); setTool('MOVE'); onMessage('Evidence returned to automatic alignment.'); }
  if (!workingAssets.supporting.length) return null;
  const toolbar: ReactNode = <div className="evidence-header-tools"><nav className="support-image-strip" aria-label="Evidence images">{workingAssets.supporting.map((asset, index) => <button type="button" key={asset.id} className={asset.id === selectedId ? 'selected' : ''} onClick={() => setSelectedId(asset.id)}><span>{images.get(asset.id) && <img src={images.get(asset.id)} alt="" />}</span><strong>{String(index + 1).padStart(2, '0')}</strong><small>{asset.name}</small></button>)}</nav><span className="evidence-toolbar-separator controls-divider" aria-hidden="true" /><button type="button" className="evidence-auto-align" onClick={resetAll}>Auto-align all</button></div>;
