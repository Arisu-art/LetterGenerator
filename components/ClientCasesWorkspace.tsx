...
export default function ClientCasesWorkspace({ records, activeCaseId, onContinue, onOpenTracker, onCreateCase }: Props) {
  ...
  return <section className="clients-cases-workspace operations-workspace">
    <section className="panel operations-command">
      <div className="operations-heading"><p className="eyebrow">Client operations</p><h2>Manage active case work</h2><p>Continue packet production or open completed work for delivery follow-up.</p></div>
      <div className="operations-actions"><button type="button" className="secondary-button" onClick={onOpenTracker}>Open Tracker</button><button type="button" className="action-button" onClick={onCreateCase}>New Case</button></div>
    </section>
    <div className="operations-metrics" aria-label="Case status summary">
      <article><small>Cases</small><strong>{records.length}</strong></article>
    ...
    <section className="panel operations-table-surface">
      <header className="operations-section-head"><div><h3>Case workspace</h3><p>Round, bureau coverage and delivery state.</p></div><span className="operations-count">{records.length} case{records.length === 1 ? '' : 's'}</span></header>
      {records.length === 0 ? <div className="operations-empty"><strong>No client cases yet</strong><p>Load source data to create the first case.</p><button type="button" className="action-button" onClick={onCreateCase}>Load Source Data</button></div> : <div className="case-records" role="list">{records.map((record) => <article key={record.id} className={`case-record ${record.id === activeCaseId ? 'current' : ''}`} role="listitem">
