...
import GuidedSourceDataFlowSafe from './GuidedSourceDataFlowSafe';
...
async function withTimeout<T>(phase: string, operation: () => Promise<T>, timeoutMs = GENERATION_TIMEOUT_MS): Promise<T> {
  ...
  try {
    return await Promise.race([operation(), new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error(`${phase} timed out after ${Math.round(timeoutMs / 1000)} seconds.`)), timeoutMs); })]);
...
export default function LetterGeneratorWorkspaceV2() {
  ...
  useEffect(() => { const storedPreferences = loadWorkspacePreferences(); setPreferences(storedPreferences); setRound(storedPreferences.defaultRound); setReferences(loadReferenceMeta()); setCases(loadClientCases()); setFilings(loadFilings()); }, []);
  ...
  function saveCase(statusValue: ClientCaseStatus, data: Partial<ClientCaseRecord> = {}) {
    const id = data.id || caseId; const name = data.clientName || parsed.name; if (!id || !name) return null;
    ...
    setCases(upsertClientCase(record)); return record;
  }
  function begin() { setRound(preferences.defaultRound); setSource(''); setOriginalSource(''); setRecoveryDraft(null); setNormalized(false); setCaseId(''); setEvidence(emptyEvidence()); clearOutputs(); report('Load a client source TXT to begin a new package.'); setPanel('Source Data'); }
  function importSource(value: string, action: string) { if (!value.trim()) return; if (source.trim()) captureDraft(`Working draft preserved before ${action.toLowerCase()} replacement`); const text = createNormalizedSourceCopy(value).text; const imported = parseSource(text); const id = crypto.randomUUID(); setOriginalSource(value); setSource(text); setNormalized(true); setCaseId(id); setEvidence(emptyEvidence()); clearOutputs(); if (imported.name) { const detected = detectRoutes(imported); setCases(upsertClientCase({ id, clientName: imported.name, round, routeCount: detected.length, bureaus: Array.from(new Set(detected.map((route) => route.bureau))), evidenceCount: 0, editableCount: 0, pdfCount: 0, status: 'SOURCE_LOCKED', updatedAt: new Date().toISOString() })); } report(`${action} source imported and protected as the original baseline.`, 'success'); }
  function standardizeDraft(value: string) { if (!value.trim()) return; const text = createNormalizedSourceCopy(value).text; const standardized = parseSource(text); const id = caseId || crypto.randomUUID(); setSource(text); setNormalized(true); if (!caseId) setCaseId(id); clearOutputs(); if (standardized.name) { const detected = detectRoutes(standardized); setCases(upsertClientCase({ id, clientName: standardized.name, round, routeCount: detected.length, bureaus: Array.from(new Set(detected.map((route) => route.bureau))), evidenceCount: evidence.supporting.length, editableCount: 0, pdfCount: 0, status: evidence.supporting.length ? 'EVIDENCE_READY' : 'SOURCE_LOCKED', updatedAt: new Date().toISOString() })); } report('Working draft standardized. The imported original remains protected and evidence has been retained.', 'success'); }
  ...
  function setLine(key: string, value: string) { const pattern = new RegExp(`^${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}:.*$`, 'im'); const line = `${key}: ${value}`; const anchor = /\n\s*\n(?=(DISPUTE ACCOUNTS|HARD INQUIRIES|LATE PAYMENTS)\b)/i; setSource(pattern.test(source) ? source.replace(pattern, line) : anchor.test(source) ? source.replace(anchor, `\n${line}\n\n`) : `${source.trim()}\n${line}`); setNormalized(true); clearOutputs(); }
  ...
  async function letter(route: LetterRoute, file: File, date: string) { const recipient = bureauInfo[route.bureau]; const identity = { consumerName: parsed.name, addressLines: parsed.address, dob: parsed.dob, ssn: parsed.ssn, letterDate: date, bureauName: recipient.name, bureauAddressLines: recipient.address.split('\n') }; return route.type === 'DISPUTE' ? renderReferenceDisputeDocx(file, { ...identity, disputeItems: route.items.filter((item) => item.type === 'DISPUTE_ACCOUNT').map((item) => item.displayText), hardInquiryItems: route.items.filter((item) => item.type === 'HARD_INQUIRY').map((item) => item.displayText) }) : renderLatePaymentReference(file, { ...identity, latePaymentItems: route.items.map((item) => item.displayText) }); }
  async function affidavit(bureau: Bureau, date: string) { const file = await readTemplateExhibit(round, 'AFFIDAVIT'); if (!file) return null; const recipient = bureauInfo[bureau]; const output = await renderMappedAppendix(file, { kind: 'AFFIDAVIT', bureau, documentDate: date, recipientName: recipient.name, recipientAddress: recipient.address, source: affidavitSource }); return highlightTextInDocx(output, { terms: [parsed.name, parsed.ssn].filter(Boolean) }); }
  async function makeZip(files: ReviewOutput[], notes: string[], date: string) { const zip = new JSZip(); await addOrderedPacketFolders(zip, { outputs: files, warnings: notes, evidence, exhibits: templates, clientName: parsed.name, round, documentDate: date }); return zip.generateAsync({ type: 'blob' }); }
  async function generate() {
    if (busy) return; setBusy(true); const date = dateNow(); const output: ReviewOutput[] = []; const notes: string[] = [];
    try { for (const route of routes) { const ref = refs.find((item) => item.type === route.type)?.file ? await readReferenceFile(refs.find((item) => item.type === route.type)!.id) : null; if (!ref) throw new Error(requiredGenerationFailureMessage([route])); report(`Generating ${labels[route.type]} for ${route.bureau}...`); const blob = await withTimeout(`Generating ${labels[route.type]}`, () => letter(route, ref, date)); output.push({ path: `Editable Documents/${clean(parsed.name)} ${route.bureau} ${labels[route.type]}.docx`, type: route.type, role: 'LETTER', sequence: 1, bureau: route.bureau, count: route.items.length, detail: route.reason, blob, packetSteps: order(route.type) }); }
      if (affidavitRequired) { const context = routes.find((item) => item.type === 'DISPUTE'); if (context) { report('Generating client Affidavit...'); const file = await withTimeout('Generating Affidavit', () => affidavit(context.bureau, date)); if (!file) throw new Error('Required component missing: 04 Affidavit.docx could not be generated.'); output.push({ id: 'CLIENT-AFFIDAVIT', path: `Editable Documents/${clean(parsed.name)} 04 ${exhibitTitles.AFFIDAVIT}.docx`, type: 'DISPUTE', role: 'AFFIDAVIT', sequence: 4, bureau: 'CLIENT', count: 1, detail: 'Shared client affidavit', blob: file, packetSteps: order('DISPUTE') }); } }
      report('Preparing complete ordered component package...'); const zip = await withTimeout('Preparing ordered package ZIP', () => makeZip(output, notes, date), ARCHIVE_TIMEOUT_MS); const zipName = `${base(parsed.name)}_${base(round)}_ORDERED_PACKET_PACKAGE.zip`; setDocs(output); setWarnings(notes); setOrderedZip({ name: zipName, blob: zip }); setDocDate(date); saveCase('REVIEW_READY', { editableCount: output.length, evidenceCount: evidence.supporting.length, pdfCount: 0 }); report('Complete ordered packet package is ready for review and download.', 'success'); setPanel('Outputs'); }
