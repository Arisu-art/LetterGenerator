...
import { userMessage } from '../lib/ui-message-contract';
...
export default function LetterGeneratorWorkspaceV2() {
  ...
  const statusMessage = useMemo(() => ({ ...userMessage({ raw: status }), tone: statusTone }), [status, statusTone]);
  const showPreflight = panel === 'Source Data' && Boolean(source.trim()) && (!preflight.ready || preflight.warnings.length > 0 || statusTone === 'error');
