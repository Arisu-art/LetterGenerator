...
export default function LetterGeneratorWorkspaceV2() {
  ...
  async function updateEvidenceDuringReview(next: PacketAssets) {
    ...
    if (reviewDocs.length) {
      const zip = await createWorkingZip(reviewDocs, warnings, documentDate || dateInEasternTime());
      setWorkingZip({ name: workingZip?.name || `${fileBase(parsed.name)}_${fileBase(round)}_WORKING_DOCUMENTS.zip`, blob: zip });
    }
