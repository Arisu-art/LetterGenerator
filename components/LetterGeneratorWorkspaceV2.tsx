...
import ClientCasesWorkspace from './ClientCasesWorkspace';
...
import { addFinalFilings, loadClientCases, loadFilings, markFilingSent, upsertClientCase, type ClientCaseRecord, type ClientCaseStatus, type FilingRecord } from '../lib/client-operations-store';
...
type Panel = 'Dashboard' | 'Clients & Cases' | 'Templates' | 'Source Data' | 'Outputs' | 'Filing Tracker' | 'Settings';
const panels: Panel[] = ['Dashboard', 'Clients & Cases', 'Templates', 'Source Data', 'Outputs', 'Filing Tracker', 'Settings'];
...
export default function LetterGeneratorWorkspaceV2() {
  ...
  useEffect(() => { setReferences(loadReferenceMeta()); setCaseRecords(loadClientCases()); setFilings(loadFilings()); }, []);
  ...
  function caseRecord(status: ClientCaseStatus, values?: { id?: string; name?: string; caseRoutes?: LetterRoute[]; evidenceCount?: number; editableCount?: number; pdfCount?: number }) {
    ...
    const record: ClientCaseRecord = {
      ...
      clientName: name,
      round,
      routeCount: trackedRoutes.length,
  ...
  function beginNewCase() {
    setSource(''); setOriginalSource(''); setNormalized(false); setCaseId(''); setEvidence(emptyEvidence()); clearOutputs(); setPanel('Source Data'); setStatus('Add client source data to begin a new case.');
  }
  ...
  async function updateEvidenceDuringReview(next: PacketAssets) {
    setEvidence(next); setFinalPackets([]); setFinalZip(null);
    if (reviewDocs.length) { const zip = await createWorkingZip(reviewDocs, warnings, documentDate || dateInEasternTime()); setWorkingZip({ name: workingZip?.name || `${fileBase(parsed.name)}_${fileBase(round)}_WORKING_DOCUMENTS.zip`, blob: zip }); }
    caseRecord(reviewDocs.length ? 'REVIEW_READY' : next.supporting.length ? 'EVIDENCE_READY' : 'SOURCE_LOCKED', { evidenceCount: next.supporting.length, pdfCount: 0 });
    setStatus('Supporting Documents layout saved. Recreate final PDFs after review.');
  }
  async function assemblePacketForRoute(type: LetterType, bureau: string, docs: ReviewOutput[]) {
    ...
    if (type === 'DISPUTE') {
      const fcra = await readTemplateExhibit(round, 'FCRA'); const attachment = await readTemplateExhibit(round, 'ATTACHMENT');
      const affidavit = docs.find((doc) => doc.bureau === bureau && doc.role === 'AFFIDAVIT'); const ftc = docs.find((doc) => doc.bureau === bureau && doc.role === 'FTC');
  ...
  async function finalizePdfPackets() {
    ...
    setFinalPackets(packets); setFinalZip(packets.length ? { name: `${fileBase(parsed.name)}_${fileBase(round)}_FINAL_PDF_PACKETS.zip`, blob: await zip.generateAsync({ type: 'blob' }) } : null); setFinalizing(false);
  ...
  function dashboard() { return <div className="dashboard-grid"><section className="panel dashboard-hero"><p className="eyebrow">Document operations</p><h2>Build ordered bureau PDF packets.</h2><p>Configure templates, load source data, review editable documents and download filing-order packets.</p><div className="dashboard-actions"><button className="action-button" onClick={() => setPanel('Clients & Cases')}>View Cases</button><button className="secondary-button" onClick={beginNewCase}>New Case</button></div></section><article className="metric-tile"><small>Cases</small><strong>{caseRecords.length}</strong><span>Tracked clients</span></article><article className="metric-tile"><small>Review docs</small><strong>{reviewDocs.length}</strong><span>Active case</span></article><article className="metric-tile"><small>Sent</small><strong>{filings.filter((item) => item.status === 'SENT').length}</strong><span>Filing records</span></article></div>; }
