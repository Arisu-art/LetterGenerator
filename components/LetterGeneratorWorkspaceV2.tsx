...
export default function LetterGeneratorWorkspaceV2() {
  ...
  function normalizeInput(value: string, action: string) { if (!value.trim()) return; setOriginalSource(value); setSource(createNormalizedSourceCopy(value).text); setNormalized(true); startCase(); setStatus(`${action} source standardized. Lock the source data to continue.`); }
  ...
  async function generateReviewDocuments() {
    ...
    setReviewDocs(docs); setWarnings(notices); setWorkingZip({ name: `${fileBase(parsed.name)}_${fileBase(round)}_WORKING_DOCUMENTS.zip`, blob: zip }); setDocumentDate(date); setFinalPackets([]); setFinalZip(null); setBusy(false); setPanel('Outputs'); setStatus(`${docs.length} editable document(s) prepared.`);
  }
  async function saveEdited(output: ReviewOutput, file: File) { const docs = reviewDocs.map((item) => item.path === output.path ? { ...item, blob: file, detail: 'Edited and saved for finalization' } : item); const zip = await createWorkingZip(docs, warnings, documentDate || dateInEasternTime()); setReviewDocs(docs); setWorkingZip({ name: workingZip?.name || 'WORKING_DOCUMENTS.zip', blob: zip }); setFinalPackets([]); setFinalZip(null); setStatus('Edit saved.'); }
  ...
  async function finalizePdfPackets() {
    ...
    setFinalPackets(packets); setFinalZip(packets.length ? { name: `${fileBase(parsed.name)}_${fileBase(round)}_FINAL_PDF_PACKETS.zip`, blob: await zip.generateAsync({ type: 'blob' }) } : null); setFinalizing(false); setStatus(packets.length ? `${packets.length} ordered final PDF packet(s) ready.` : 'No final packets are available.');
  }
  ...
  function sourceView() { return <GuidedSourceDataFlow source={source} originalSource={originalSource} normalized={normalized} verified={verified} parsed={parsed} routes={routes} sourceWarnings={sourceWarnings} evidenceKey={evidenceKey} evidence={evidence} canGenerate={canGenerate} onNormalize={normalizeInput} onEditSource={(value) => { setSource(value); setNormalized(false); clearOutputs(); }} onRestore={() => { setSource(originalSource); setNormalized(false); setCaseId(''); setEvidence(emptyEvidence()); clearOutputs(); }} onEvidenceChanged={(next) => { setEvidence(next); clearOutputs(); }} onMessage={setStatus} onContinue={() => setPanel('Generate')} />; }
