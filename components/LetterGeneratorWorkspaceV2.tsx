...
export default function LetterGeneratorWorkspaceV2() {
  ...
  async function generateReviewDocuments() {
    if (!canGenerate || (preferences.strictValidation && missingLetters.length)) { setStatus('Complete required generation checks first.'); return; }
  ...
  async function finalizePdfPackets() {
    if (preferences.requireEvidenceForFinalPdf && !evidence.supporting.length) { setStatus('Supporting Documents evidence is required before final PDF creation in Settings.'); return; }
  ...
  function dashboard() { return <DashboardOperationsWorkspace cases={caseRecords} filings={filings} activeCaseId={caseId} onNewCase={beginNewCase} onOpenCases={() => setPanel('Clients & Cases')} onOpenTemplates={() => setPanel('Templates')} onOpenSource={() => setPanel('Source Data')} onOpenOutputs={() => setPanel(workingZip ? 'Outputs' : 'Clients & Cases')} onOpenTracker={() => setPanel('Filing Tracker')} />; }
