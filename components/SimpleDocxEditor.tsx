...
function documentRole(output: ReviewOutput) { return output.role === 'AFFIDAVIT' ? 'Affidavit' : output.role === 'FTC' ? 'FTC Identity Theft Report' : output.type === 'DISPUTE' ? 'Dispute Letter' : 'Late Payment Letter'; }
function stepNumber(step: string) { return Number(step.match(/^\s*(\d+)/)?.[1] || 0); }
...
export default function SimpleDocxEditor({ output, onClose, onSave, onPreviewPacket }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([readEditableParagraphs(output.blob), import('docx-preview')]).then(async ([paragraphs, visual]) => {
      ...
      setStatus('Edit this DOCX or open Complete Packet Preview to see all packet pages merged in final order.');
  ...
  async function previewCompletePacket() {
    ...
    try {
      ...
      setPreview(packet);
      setView('PACKET');
      setStatus('Complete packet preview loaded. Inspect every ordered page here before downloading.');
    } catch (error) { setPreview(null); setStatus(error instanceof Error ? error.message : 'Complete packet preview could not be prepared.'); }
