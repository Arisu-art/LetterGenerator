...
function EditablePacketSection({ slot, onSave }: { slot: Slot; onSave: Props['onSave'] }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([readEditableParagraphs(output.blob), import('docx-preview')]).then(async ([items, docx]) => {
      ...
      await docx.renderAsync(await output.blob.arrayBuffer(), host.current, undefined, {
        className: 'packet-inline-docx', inWrapper: true, ignoreWidth: false, ignoreHeight: false,
        breakPages: true, renderHeaders: false, renderFooters: false
      });
      if (!alive || !host.current) return;
      ...
      items.forEach((item, index) => {
        ...
        node.addEventListener('input', () => {
          setDirty(true);
          setParagraphs((current) => current.map((entry) => entry.id === item.id ? { ...entry, text: textOf(node), dirty: true } : entry));
        });
      });
  ...
  async function save() {
    ...
    try {
      ...
      setDirty(false);
      setStatus('Saved to packet');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Save failed.');
    } finally { setSaving(false); }
  }

  return <article className="packet-stack-section packet-stack-editable" data-slot={slot.id}>
    <header className="packet-stack-header"><b>{String(slot.number).padStart(2, '0')}</b><div><h3>{slot.label}</h3><p>{output.bureau} · {status}</p></div><button disabled={!dirty || saving} onClick={() => void save()}>{saving ? 'Saving...' : dirty ? 'Save Section' : 'Saved'}</button></header>
    <div ref={host} className="packet-inline-docx-host" />
  </article>;
}

function PacketInsertSection({ slot }: { slot: Slot }) {
  const supporting = slot.id === 'SUPPORTING';
  const state = supporting ? 'Layout editor' : slot.configured ? 'Configured' : 'None';
  return <article className="packet-stack-section packet-stack-insert" data-slot={slot.id}>
    <header className="packet-stack-header"><b>{String(slot.number).padStart(2, '0')}</b><div><h3>{slot.label}</h3><p>{slot.message}</p></div><span className={`packet-stack-state ${supporting ? 'managed' : slot.configured ? 'ready' : 'none'}`}>{state}</span></header>
    <div className={`packet-insert-status ${supporting ? 'managed' : slot.configured ? '' : 'missing'}`}>
      <strong>{supporting ? 'Supporting Documents one-page layout' : slot.configured ? `${slot.label} is included here.` : 'None'}</strong>
      <span>{supporting ? 'Crop, move, and auto-align evidence images from Source Data; the saved layout is used at this position.' : slot.configured ? 'Replace this insert from Templates when needed.' : 'Not configured for this packet position.'}</span>
    </div>
  </article>;
}

export default function SimpleDocxEditor({ round, output, documents, onClose, onSave }: Props) {
  ...
  const slots: Slot[] = output.type === 'DISPUTE' ? [
    ...
    { id: 'SUPPORTING', number: 2, label: 'Supporting Documents', message: 'Editable image layout from Source Data' },
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      const id = visible?.target.getAttribute('data-slot') as SlotId | null;
      if (id) setActive(id);
  ...
  function jump(id: SlotId) {
    scroll.current?.querySelector<HTMLElement>(`[data-slot="${id}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  return <div className="simple-editor-backdrop"><section className="simple-editor-modal ordered-packet-modal" role="dialog" aria-modal="true" aria-label={`${output.bureau} ordered packet editor`}>
    <header className="simple-editor-header"><div><p className="eyebrow">Editable round packet</p><h2>{output.bureau} {output.type === 'DISPUTE' ? 'Dispute Packet' : 'Late Payment Packet'}</h2><span>{round} · Standard filing order · editable components appear inline</span></div><button className="close-editor" onClick={onClose} aria-label="Close editor">×</button></header>
    <div className="editor-view-switch document-mode"><strong>Ordered Packet Editor</strong><span>Scroll one bureau packet; final PDF creation occurs only when requested from Outputs.</span></div>
    <div className="ordered-packet-body"><aside className="editor-packet-map document-rail"><header><p className="eyebrow">Standard order</p><h3>{output.bureau}</h3><span>Each position remains visible in this {round} packet.</span></header><ol>{slots.map((slot) => <li className={active === slot.id ? 'current editable' : 'editable'} key={slot.id}><button type="button" onClick={() => jump(slot.id)}><b>{String(slot.number).padStart(2, '0')}</b><div><strong>{slot.label}</strong><small>{slot.document ? 'Editable DOCX' : slot.id === 'SUPPORTING' ? 'One-page layout' : slot.configured ? 'Configured insert' : 'None'}</small></div></button></li>)}</ol><p className="map-instruction">Letter, Affidavit and FTC save inline. Arrange Supporting Documents in Source Data.</p></aside><div ref={scroll} className="ordered-packet-scroll">{slots.map((slot) => slot.document ? <EditablePacketSection key={slot.id} slot={slot} onSave={onSave} /> : <PacketInsertSection key={slot.id} slot={slot} />)}</div></div>
  </section></div>;
}
