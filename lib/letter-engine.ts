...
function sectionOf(value: string): Section | '' {
  ...
  if (/^(FTC\s+IDENTITY\s+THEFT\s+REPORT|FTC\s+AFFECTED\s+ACCOUNTS|AFFECTED\s+ACCOUNTS)$/.test(key)) return 'ftc';
...
function isSectionHeading(value: string, section: Section) {
  ...
  if (section === 'ftc') return /^(FTC\s+IDENTITY\s+THEFT\s+REPORT|FTC\s+AFFECTED\s+ACCOUNTS|AFFECTED\s+ACCOUNTS)$/.test(key);
...
export function parseSource(text: string): ParsedSource {
  ...
  parsed.ftcReportNumber = headerField(headerLines, /^FTC\s+REPORT\s+NUMBER\s*:\s*/i); parsed.ftcReportDate = headerField(headerLines, /^FTC\s+REPORT\s+DATE\s*:\s*/i);
...
export function createNormalizedSourceCopy(source: string): NormalizedSourceCopy {
  ...
  if (parsed.ftcReportNumber || parsed.ftcReportDate || parsed.ftcAccounts.length) sections.push(`FTC REPORT NUMBER: ${parsed.ftcReportNumber}`, `FTC REPORT DATE: ${parsed.ftcReportDate}`, '', 'FTC AFFECTED ACCOUNTS', ...ftcLines(parsed.ftcAccounts));
