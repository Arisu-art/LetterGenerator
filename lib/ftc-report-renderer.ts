...
function paragraph(text = '', options: { bold?: boolean; size?: number; align?: 'left' | 'center' | 'right'; fill?: string; after?: number } = {}) {
  ...
  return `<w:p><w:pPr>${align}${fill}<w:spacing w:after="${after}"/></w:pPr><w:r><w:rPr>${bold}<w:sz w:val="${size}"/><w:szCs w:val="${size}"/></w:rPr><w:t xml:space="preserve">${escapeXml(text)}</w:t></w:r></w:p>`;
}

function sectionTitle(text: string) {
  return paragraph(text, { bold: true, fill: 'F8CBAD', after: 140 });
}

function cell(text: string, width = 2500, shaded = false) {
  return `<w:tc><w:tcPr><w:tcW w:w="${width}" w:type="dxa"/><w:tcBorders><w:top w:val="single" w:sz="6"/><w:left w:val="single" w:sz="6"/><w:bottom w:val="single" w:sz="6"/><w:right w:val="single" w:sz="6"/></w:tcBorders>${shaded ? '<w:shd w:fill="D9D9D9"/>' : ''}</w:tcPr>${String(text || '').split('\n').map((line) => paragraph(line, { after: 40 })).join('')}</w:tc>`;
}

function table(rows: string[][], widths?: number[], headerRows = new Set<number>()) {
  return `<w:tbl><w:tblPr><w:tblW w:w="10000" w:type="dxa"/><w:tblBorders><w:top w:val="single" w:sz="6"/><w:left w:val="single" w:sz="6"/><w:bottom w:val="single" w:sz="6"/><w:right w:val="single" w:sz="6"/><w:insideH w:val="single" w:sz="6"/><w:insideV w:val="single" w:sz="6"/></w:tblBorders></w:tblPr>${rows.map((row, rowIndex) => `<w:tr>${row.map((value, index) => cell(value, widths?.[index] || Math.floor(10000 / row.length), headerRows.has(rowIndex))).join('')}</w:tr>`).join('')}</w:tbl>`;
}
...
function monthYear(value: string) {
  ...
  return `${Number(dateMatch[1])}/${year}`;
}

function reportMonthYear(source: ParsedSource) {
  const sourceDate = (source as any).ftcReportDate || new Date().toLocaleDateString('en-US');
  return monthYear(sourceDate);
}

function deriveAccountFromDisplay(displayText: string, defaultFraudBegan: string): FtcAffectedAccount | null {
  ...
  const amountDate = lines.join(' ').match(/(?:^|\s)(\d{2,7}(?:\.\d{1,2})?)?\s*((?:0?[1-9]|1[0-2])\/(?:19|20)?\d{2})(?:\s|$)/);
  ...
  return {
    accountName,
    accountNumber,
    fraudBegan: defaultFraudBegan,
    dateDiscovered: amountDate?.[2] ? monthYear(amountDate[2]) : '',
    fraudulentAmount: amountDate?.[1] || ''
  };
}

function deriveInquiryFromDisplay(displayText: string, defaultFraudBegan: string): FtcAffectedAccount | null {
  const match = displayText.trim().replace(/\s*[–—]\s*/g, ' - ').match(/^(.+?)\s+-\s+(\d{1,2}\/\d{1,2}\/\d{2,4})$/);
...
export function buildFtcAffectedAccounts(source: ParsedSource): FtcAffectedAccount[] {
  const defaultFraudBegan = reportMonthYear(source);
  const explicit = Array.isArray((source as any).ftcAccounts) ? (source as any).ftcAccounts : [];

  const disputeItems = Object.values((source as any).dispute || {})
    .flat()
    .map((item: any) => deriveAccountFromDisplay(item.displayText || '', defaultFraudBegan))
    .filter(Boolean) as FtcAffectedAccount[];

  const inquiryItems = Object.values((source as any).inquiry || {})
    .flat()
    .map((item: any) => deriveInquiryFromDisplay(item.displayText || '', defaultFraudBegan))
    .filter(Boolean) as FtcAffectedAccount[];

  const seen = new Set<string>();

  return [...explicit, ...disputeItems, ...inquiryItems]
    .map((item: FtcAffectedAccount) => ({
      accountName: item.accountName || '',
      accountNumber: item.accountNumber || '',
      fraudBegan: item.fraudBegan || defaultFraudBegan,
      dateDiscovered: item.dateDiscovered || '',
      fraudulentAmount: item.fraudulentAmount || ''
    }))
    .filter((item: FtcAffectedAccount) => {
...
function accountBlock(account: FtcAffectedAccount) {
  return [
    paragraph('Credit Card opened by the thief', { bold: true, after: 80 }),
    table([
...
export async function renderFtcIdentityTheftReportDocx(source: ParsedSource, documentDate: string) {
  ...
  const address = Array.isArray((source as any).address) ? [...(source as any).address, (source as any).country || 'USA'].filter(Boolean).join('\n') : '';
  const phone = ((source as any).phone || '').replace(/[()]/g, '').replace(/\s+/g, '-').replace(/--+/g, '-');
  const firstName = (source as any).firstName || name.split(/\s+/)[0] || '';
  const lastName = (source as any).lastName || name.split(/\s+/).slice(1).join(' ') || '';
  const reportNumber = (source as any).ftcReportNumber || '';
...
${table([
  ...
  [firstName, (source as any).middleName || '', lastName],
  ['Address:', 'Phone:', 'Email:'],
  [address, phone, (source as any).email || '']
