...
const storageKey = 'lettergenerator.visual-reference-output.v11';
const legacyKeys = ['lettergenerator.visual-reference-output.v10', 'lettergenerator.reference-accurate-letters.v9', 'lettergenerator.category-letters.v8', 'lettergenerator.reference-canvas.v6', 'lettergenerator.round.library.v5'];
...
function mergeSaved(raw: unknown): ReferenceSlot[] {
  ...
  const documents = raw.flatMap((item: { docs?: Array<{ id: string; file?: string; size?: number }> }) => item.docs || []);
  return slots.map((slot) => {
    const current = raw.find((item: ReferenceSlot) => item.id === slot.id && typeof item.file === 'string') as ReferenceSlot | undefined;
    const previous = documents.find((item) => item.id === slot.id);
    return current ? { ...slot, file: current.file, size: current.size } : previous ? { ...slot, file: previous.file || '', size: previous.size } : slot;
  });
}
...
export default function Page() {
  ...
  async function uploadSlot(file: File) { if (!isDocx(file.name)) { setStatus('Only DOCX documents are accepted.'); return; } await putFile(selected.id, file); setSlots((items) => items.map((slot) => slot.id === selected.id ? { ...slot, file: file.name, size: file.size } : slot)); setStatus(`${selected.name} saved. It will control that document type's generated appearance.`); }
