...
function seedLibrary(): Packet[] {
  return rounds.flatMap((round, index) => {
    const disputePrefix = index === 0 ? 'dispute' : `r${index + 1}-dispute`;
    const latePrefix = index === 0 ? 'late' : `r${index + 1}-late`;
    return [
...
function mergeLegacyFirstRound(library: Packet[], legacy: Partial<Packet>[]) {
  return library.map((packet) => {
    if (packet.round !== '1st Round') return packet;
    const saved = legacy.find((item) => item.kind === packet.kind);
    if (!saved || !saved.docs) return packet;
    return { ...packet, name: saved.name || packet.name, description: saved.description || packet.description, docs: packet.docs.map((doc) => ({ ...doc, ...(saved.docs?.find((old) => old.id === doc.id) || {}) })) };
  });
}
  ...
  useEffect(() => {
    try {
      ...
      if (stored) { setPackets(JSON.parse(stored) as Packet[]); setStatus('All saved round templates restored.'); }
        ...
        if (legacy) { setPackets((current) => mergeLegacyFirstRound(current, JSON.parse(legacy) as Partial<Packet>[])); setStatus('Your saved 1st Round templates were preserved and migrated into the round library.'); }
      }
  ...
  async function generate() {
    ...
    for (const route of routes) {
      ...
      for (const doc of selectedPacket.docs.filter((item) => item.file)) {
        const original = await retrieveFile(doc.id); if (!original) continue;
