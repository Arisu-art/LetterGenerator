...
export default function Page() {
  ...
  function manifest(files: Output[], warnings: string[], date: string, notes: string[]) {
    return ['LetterGenerator Visual Reference Manifest', `Client: ${parsed.name}`, `Round: ${round}`, `Letter date (US Eastern): ${date}`, '', 'Output Decisions:', ...bureaus.flatMap((bureau) => { const dispute = parsed.dispute[bureau].length; const inquiry = parsed.inquiry[bureau].length; const late = parsed.late[bureau].length; return [`${dispute || inquiry ? 'CREATE' : 'SKIP'} | Dispute | ${bureau} | ${dispute} dispute account(s), ${inquiry} hard inquiry item(s)`, `${late ? 'CREATE' : 'SKIP'} | Late Payment | ${bureau} | ${late} item(s)`]; }), '', 'Files Included in Current Package:', ...files.map((file) => `- ${file.path}`), ...(notes.length ? ['', 'Review Changes:', ...notes.map((note) => `- ${note}`)] : []), ...(warnings.length ? ['', 'Generation Warnings:', ...warnings.map((warning) => `- ${warning}`)] : [])].join('\n');
  }
  ...
  async function replaceReviewedDocx(output: Output, file: File) {
    if (!isDocx(file.name)) { setStatus('Review replacement must be a DOCX file.'); return; }
    const updated = outputs.map((item) => item.path === output.path ? { ...item, blob: file, detail: 'Reviewed DOCX replacement' } : item);
    const notes = [...reviewNotes, `REPLACED | ${output.path} | ${file.name}`];
    const packed = await buildZip(updated, generationWarnings, generatedDate || currentUsLetterDate(), notes);
    setOutputs(updated); setReviewNotes(notes); setZipOutput({ name: zipOutput?.name || `${safePackageName(parsed.name)}_${safePackageName(round)}_LETTERS.zip`, blob: packed }); setStatus(`${output.path.split('/').pop()} replaced in the ZIP package.`);
  }
