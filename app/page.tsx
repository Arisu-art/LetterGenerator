...
function seedSlots(): ReferenceSlot[] {
  return rounds.flatMap((round, index) => {
    ...
    return [{ id: `${prefix}dispute-letter`, round, type: 'DISPUTE', name: `${round} Dispute Output Reference`, file: '' }, { id: `${prefix}late-letter`, round, type: 'LATE_PAYMENT', name: `${round} Late Payment Output Reference`, file: '' }];
  });
}
...
export default function Page() {
  ...
  useEffect(() => {
    try {
      ...
      else for (const key of legacyKeys) { const previous = localStorage.getItem(key); if (previous) { setSlots(mergeSaved(JSON.parse(previous))); setStatus('Earlier uploaded document restored. Replace each slot with its finished DOCX output reference.'); break; } }
  ...
  function chooseRound(next: Round) { setRound(next); setSelectedId(slots.find((slot) => slot.round === next)!.id); setOutputs([]); setGenerationWarnings([]); setZipOutput(null); }
  ...
  async function deleteSlot() { if (!window.confirm(`Delete ${selected.name}?`)) return; await removeFile(selected.id); setSlots((items) => items.map((slot) => slot.id === selected.id ? { ...slot, file: '', size: undefined } : slot)); setStatus(`${selected.name} removed.`); }
  async function renderRoute(route: LetterRoute, input: File, letterDate: string) {
    ...
    if (route.type === 'DISPUTE') return renderReferenceDisputeDocx(input, { ...identity, fraudItems: route.items.map((item) => item.displayText) });
  ...
  async function generate() {
    ...
    setLoading(true);
    const zip = new JSZip(); const made: Output[] = []; const warnings: string[] = []; const date = currentUsLetterDate();
    for (const route of routes) {
      ...
      try {
        ...
        zip.file(path, blob); made.push({ path, type: route.type, bureau: route.bureau, count: route.items.length, detail: 'Completed reference format used', blob });
      } catch (error) { warnings.push(`${label[route.type]} / ${route.bureau}: ${error instanceof Error ? error.message : 'rendering failed.'}`); }
    }
    zip.file('Generation Manifest.txt', ['LetterGenerator Visual Reference Manifest', `Client: ${parsed.name}`, `Round: ${round}`, `Letter date (US Eastern): ${date}`, '', 'Output Decisions:', ...bureaus.flatMap((bureau) => { const dispute = parsed.dispute[bureau].length; const inquiry = parsed.inquiry[bureau].length; const late = parsed.late[bureau].length; return [`${dispute || inquiry ? 'CREATE' : 'SKIP'} | Dispute | ${bureau} | ${dispute} dispute account(s), ${inquiry} hard inquiry item(s)`, `${late ? 'CREATE' : 'SKIP'} | Late Payment | ${bureau} | ${late} item(s)`]; }), '', 'Created Files:', ...made.map((output) => `- ${output.path}`), ...(warnings.length ? ['', 'Warnings:', ...warnings.map((warning) => `- ${warning}`)] : [])].join('\n'));
    const packed = await zip.generateAsync({ type: 'blob' });
    setOutputs(made); setGenerationWarnings(warnings); setZipOutput({ name: `${safePackageName(parsed.name)}_${safePackageName(round)}_LETTERS.zip`, blob: packed }); setPanel('Outputs'); setLoading(false); setStatus(warnings.length ? `${made.length} DOCX created. ${warnings.length} output(s) require attention.` : `${made.length} DOCX letter(s) created successfully.`);
  }
  ...
  function workflowNav() {
    ...
    return <nav className="workflow-rail" aria-label="Workflow steps">{workflow.map((item, index) => { const disabled = !panelEnabled(item); return <button key={item} disabled={disabled} aria-disabled={disabled} className={item === panel ? 'current' : step >= 0 && index < step ? 'complete' : ''} onClick={() => setPanel(item)} title={disabled ? item === 'Generate' ? 'Load source data to unlock Generate.' : 'Generate a package to unlock Outputs.' : ''}><i>{index + 1}</i><span>{item}</span></button>; })}</nav>;
  }
  function dashboardView() {
    return <div className="dashboard-grid"><section className="panel dashboard-hero"><p className="eyebrow">Operations control center</p><h2>Generate precise bureau letters from verified references.</h2><p>Manage your source, reference documents, detected routes and output packages from one focused workspace.</p><div className="dashboard-actions"><button className="action-button" onClick={() => setPanel('Source Data')}>Import Source</button><button className="secondary-button" onClick={() => setPanel('Templates')}>Manage References</button></div></section><section className="panel dashboard-readiness"><div className="panel-heading"><div><h2>Current readiness</h2><p>{round} production checks</p></div><Pill tone={generationReady ? 'success' : 'warning'}>{generationReady ? 'Ready' : 'Setup'}</Pill></div><div className="status-stack"><div><span>Source data</span><Pill tone={sourceReady ? 'success' : 'warning'}>{sourceReady ? 'Loaded' : 'Required'}</Pill></div><div><span>Reference files</span><Pill tone={uploadedReferences === 2 ? 'success' : 'warning'}>{uploadedReferences}/2</Pill></div><div><span>Required outputs</span><strong>{routes.length}</strong></div><div><span>Missing references</span><strong>{missing.length}</strong></div></div></section><article className="metric-tile"><small>Client</small><strong>{parsed.name || 'No source loaded'}</strong><span>{sourceReady ? 'Detected from source' : 'Upload TXT to begin'}</span></article><article className="metric-tile"><small>Detected letters</small><strong>{routes.length}</strong><span>{routes.length ? 'Ready for verification' : 'No routes assessed'}</span></article><article className="metric-tile"><small>Saved references</small><strong>{uploadedReferences}<em>/2</em></strong><span>{round}</span></article><article className="metric-tile"><small>Generated DOCX</small><strong>{outputs.length}</strong><span>{zipOutput ? 'Package available' : 'Awaiting generation'}</span></article><section className="panel dashboard-routes"><div className="panel-heading"><div><h2>Detected output plan</h2><p>Bureau-specific documents required from the current source.</p></div>{generationReady && <button className="text-action" onClick={() => setPanel('Generate')}>Open generation →</button>}</div>{routes.length ? <div className="route-list">{routes.map((route) => <div className="route-card" key={`${route.type}-${route.bureau}`}><strong>{route.bureau}</strong><span>{label[route.type]}</span><small>{route.reason}</small></div>)}</div> : <Empty title="No source assessed" text="Import a TXT source file to detect dispute and late-payment output routes." />}</section></div>;
  }
  function templatesView() { return <div className="content-grid"><section className="panel"><div className="panel-heading"><div><h2>Completed output references</h2><p>Upload one finished DOCX reference per letter type and per round.</p></div><Pill tone="accent">{round}</Pill></div>{roundTabs()}<div className="documents">{roundSlots.map((slot, index) => <button key={slot.id} className={`document ${slot.id === selected.id ? 'selected' : ''}`} onClick={() => setSelectedId(slot.id)}><i>{index + 1}</i><span><strong>{slot.name}</strong><small>{slot.file || 'Finished output reference DOCX required'}</small></span><Pill tone={slot.file ? 'success' : 'warning'}>{slot.file ? 'Saved' : 'Needed'}</Pill></button>)}</div></section><section className="panel editor-panel"><div className="panel-heading"><div><h2>{selected.name}</h2><p>Completed document reference mode</p></div><Pill tone={selected.file ? 'success' : 'warning'}>{selected.file ? 'Saved' : 'Needed'}</Pill></div>{selected.file ? <div className="saved-file"><strong>{selected.file}</strong><span>{bytes(selected.size)} · DOCX</span><p>The output uses this finished document&apos;s content regions and formatting.</p></div> : <div className="upload-empty"><p>Upload a completed document that already looks exactly correct for this letter type.</p></div>}<label className="field-label">Upload / replace finished DOCX<input className="file-input" type="file" accept=".docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadSlot(file); event.target.value = ''; }} /></label>{selected.file && <button className="delete-button" onClick={() => void deleteSlot()}>Delete saved file</button>}<div className="info-card"><strong>{selected.type === 'DISPUTE' ? 'Dispute reference content required' : 'Late Payment reference content required'}</strong><p>{selected.type === 'DISPUTE' ? 'Must include the client block, date, bureau block, fraudulent-items heading, one styled item example and the signature section.' : 'Must include the client block, date, bureau block, a visible late-payment items section, one correctly formatted creditor/account example and the signature section.'}</p></div></section></div>; }
  function sourceView() {
    return <div className="content-grid"><section className="panel"><div className="panel-heading"><div><h2>Inspect source</h2><p>Data is detected by bureau before output is created. Letter date is inserted automatically using US Eastern date.</p></div></div><label className="field-label">Upload TXT source<input className="file-input" type="file" accept=".txt" onChange={async (event) => { const file = event.target.files?.[0]; if (file) { setSource(await file.text()); setOutputs([]); setGenerationWarnings([]); setZipOutput(null); setStatus('Source read. Matching outputs are now available for generation.'); } }} /></label><textarea className="source-area" value={source} onChange={(event) => { setSource(event.target.value); setOutputs([]); setGenerationWarnings([]); setZipOutput(null); }} placeholder="Paste TXT source here…" /></section>{sourceReady ? <section className="panel"><div className="panel-heading"><div><h2>Output decision canvas</h2><p>One letter per detected bureau and letter type.</p></div><Pill tone="accent">{round}</Pill></div>{roundTabs()}<div className="route-table">{bureaus.map((bureau) => { const dispute = parsed.dispute[bureau].length + parsed.inquiry[bureau].length; const late = parsed.late[bureau].length; return <div className="bureau" key={bureau}><strong>{bureau}</strong><Pill tone={dispute ? 'success' : 'neutral'}>{dispute ? `Dispute ${dispute}` : 'No Dispute'}</Pill><Pill tone={late ? 'success' : 'neutral'}>{late ? `Late ${late}` : 'No Late'}</Pill></div>; })}</div>{routes.length > 0 && <div className="pending-list">{routes.map((route) => <span key={`${route.type}-${route.bureau}`}><strong>{label[route.type]} / {route.bureau}</strong> — {route.reason}</span>)}</div>}<button className="action-button" disabled={!generationReady} onClick={() => setPanel('Generate')}>Continue with {round}</button></section> : <section className="panel idle-panel"><Empty title="Awaiting source data" text="Upload or paste a client TXT source. Bureau decisions and round actions appear after valid data is detected." /></section>}</div>;
  }
  function generateView() {
    if (!generationReady) return <section className="panel idle-panel"><Empty title="Generate is locked" text="Load valid source data first. Document routes appear only after the source has been assessed." /></section>;
    return <div className="content-grid"><section className="panel"><div className="panel-heading"><div><h2>Generate {round}</h2><p>Uses completed reference layout for dispute and late-payment documents.</p></div><Pill tone="accent">{routes.length} letters</Pill></div><div className="route-list">{routes.map((route) => <div className="route-card" key={`${route.type}-${route.bureau}`}><strong>{route.bureau}</strong><span>{label[route.type]}</span><small>{route.reason}</small></div>)}</div><button className="action-button" disabled={loading} onClick={() => void generate()}>{loading ? 'Rendering DOCX letters…' : 'Generate DOCX Letters ZIP'}</button></section><section className="panel"><div className="panel-heading"><div><h2>Generation readiness</h2><p>Only requirements affecting this package are shown.</p></div></div>{missing.length ? <div className="pending-list"><p>Reference files required for detected outputs:</p>{missing.map((route) => <span key={`${route.type}-${route.bureau}`}>{label[route.type]} / {route.bureau}</span>)}</div> : <div className="alert success">All detected letter types have an uploaded reference for {round}.</div>}{blockers.map((item) => <div className="alert error" key={item}>{item}</div>)}</section></div>;
  }
  function outputsView() { return <section className="panel outputs"><div className="panel-heading"><div><h2>{round} output package</h2><p>DOCX files in flat category folders.</p></div><Pill tone={generationWarnings.length ? 'warning' : zipOutput ? 'success' : 'neutral'}>{outputs.length} DOCX</Pill></div>{zipOutput && <div className="info-card" style={{ marginBottom: 18 }}><strong>{zipOutput.name}</strong><p>{generationWarnings.length ? 'Package created with document outputs requiring attention below.' : 'Includes generated DOCX files and a decision manifest.'}</p><button className="secondary-button" style={{ marginTop: 12 }} onClick={() => download(zipOutput.name, zipOutput.blob)}>Download ZIP Package</button></div>}{outputs.length ? <div className="output-list">{outputs.map((output) => <article className="output" key={output.path}><div><Pill tone="success">Rendered DOCX</Pill><h3>{output.path}</h3><p>{output.count} item block(s) · {output.detail}</p></div></article>)}</div> : <Empty title="No generated DOCX" text="Generate a package after uploading reference documents and source data." />}{generationWarnings.length > 0 && <div className="failed-output-list">{generationWarnings.map((warning) => <article className="failed-output" key={warning}><strong>Not generated</strong><p>{warning}</p></article>)}</div>}</section>; }
