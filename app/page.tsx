...
export default function Page() {
  ...
  async function generate() {
    ...
    setOutputs(made); setZipOutput({ name: `${safePackageName(parsed.name)}_${safePackageName(round)}_LETTERS.zip`, blob: packed }); setPanel('Outputs'); setLoading(false); setStatus(`${made.length} DOCX letter(s) created. Each DOCX is named using client name and bureau name.`);
  }
  function roundTabs() {
  ...
  function workflowNav() {
    ...
    return <nav className="workflow-rail" aria-label="Workflow steps">{workflow.map((item, index) => <button key={item} className={item === panel ? 'current' : step >= 0 && index < step ? 'complete' : ''} onClick={() => setPanel(item)}><i>{index + 1}</i><span>{item}</span></button>)}</nav>;
  }
  function primaryHeaderAction() {
    if (panel === 'Dashboard') return <button className="header-action" onClick={() => setPanel(sourceReady ? 'Generate' : 'Source Data')}>{sourceReady ? 'Review Generation' : 'Import Source'}</button>;
    if (panel === 'Templates') return <button className="header-action" onClick={() => setPanel('Source Data')}>Import Source</button>;
    if (panel === 'Source Data') return <button className="header-action" onClick={() => setPanel('Generate')}>Review Generation</button>;
    return null;
  }
  function dashboardView() {
    return <div className="dashboard-grid">
      ...
      <section className="panel dashboard-readiness"><div className="panel-heading"><div><h2>Current readiness</h2><p>{round} production checks</p></div><Pill tone={sourceReady && routes.length ? 'success' : 'warning'}>{sourceReady && routes.length ? 'Ready' : 'Setup'}</Pill></div><div className="status-stack"><div><span>Source data</span><Pill tone={sourceReady ? 'success' : 'warning'}>{sourceReady ? 'Loaded' : 'Required'}</Pill></div><div><span>Reference files</span><Pill tone={uploadedReferences === 2 ? 'success' : 'warning'}>{uploadedReferences}/2</Pill></div><div><span>Required outputs</span><strong>{routes.length}</strong></div><div><span>Missing references</span><strong>{missing.length}</strong></div></div></section>
      <article className="metric-tile"><small>Client</small><strong>{parsed.name || 'No source loaded'}</strong><span>{sourceReady ? 'Detected from source' : 'Upload TXT to begin'}</span></article>
      <article className="metric-tile"><small>Detected letters</small><strong>{routes.length}</strong><span>{routes.length ? 'Ready for verification' : 'No routes assessed'}</span></article>
      <article className="metric-tile"><small>Saved references</small><strong>{uploadedReferences}<em>/2</em></strong><span>{round}</span></article>
      <article className="metric-tile"><small>Generated DOCX</small><strong>{outputs.length}</strong><span>{zipOutput ? 'Package available' : 'Awaiting generation'}</span></article>
      <section className="panel dashboard-routes"><div className="panel-heading"><div><h2>Detected output plan</h2><p>Bureau-specific documents required from the current source.</p></div>{routes.length > 0 && <button className="text-action" onClick={() => setPanel('Generate')}>Open generation →</button>}</div>{routes.length ? <div className="route-list">{routes.map((route) => <div className="route-card" key={`${route.type}-${route.bureau}`}><strong>{route.bureau}</strong><span>{label[route.type]}</span><small>{route.reason}</small></div>)}</div> : <Empty title="No source assessed" text="Import a TXT source file to detect dispute and late-payment output routes." />}</section>
    </div>;
  }
