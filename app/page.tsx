...
type Parsed = { name: string; address: string[]; dob: string; ssn: string; inquiries: string[]; dispute: Record<Bureau, string[]>; late: Record<Bureau, string[]> };
type Route = { bureau: Bureau; kind: Kind; records: string[] };
type Output = { id: string; name: string; packet: string; document: string; bureau: Bureau; rendered: boolean; detail: string; blob: Blob };
...
const panels: { id: Panel; title: string; subtitle: string }[] = [
  ...
  { id: 'Source Data', title: 'Source Data', subtitle: 'Import and route' },
  { id: 'Generate', title: 'Generate', subtitle: 'Render output files' },
  { id: 'Outputs', title: 'Outputs', subtitle: 'Download files' },
  { id: 'Settings', title: 'Settings', subtitle: 'Preferences' }
];
const workflow: Panel[] = ['Templates', 'Source Data', 'Generate', 'Outputs'];
const metadataKey = 'lettergenerator.round.library.v4';
const legacyKeys = ['lettergenerator.round.library.v3', 'lettergenerator.first-round.library.v2'];
...
function mergeSaved(seed: Packet[], stored: Packet[]) {
  return seed.map((packet) => {
    ...
    return { ...packet, docs: packet.docs.map((doc) => ({ ...doc, ...(saved.docs.find((item) => item.id === doc.id) || {}) })) };
  });
}
...
function blank(): Parsed { return { name: '', address: [], dob: '', ssn: '', inquiries: [], dispute: { TRANSUNION: [], EQUIFAX: [], EXPERIAN: [] }, late: { TRANSUNION: [], EQUIFAX: [], EXPERIAN: [] } }; }
function parseSource(text: string): Parsed {
  const data = blank(); const header: string[] = []; let section = 'header'; let bureau: Bureau | '' = '';
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (section === 'inquiry') data.inquiries.push(line);
  }
...
export default function Page() {
  ...
  const [status, setStatus] = useState('Upload a DOCX template containing approved placeholders.');
  useEffect(() => { try { let saved: Packet[] | null = null; const current = localStorage.getItem(metadataKey); if (current) saved = JSON.parse(current) as Packet[]; if (!saved) for (const key of legacyKeys) { const value = localStorage.getItem(key); if (value) { saved = JSON.parse(value) as Packet[]; break; } } if (saved) { setLibrary(mergeSaved(seedLibrary(), saved)); setStatus('Saved templates restored.'); } } catch { setStatus('Template library ready.'); } setRestored(true); }, []);
  ...
  const blockers = [!source.trim() ? 'Upload or paste TXT source data.' : '', source.trim() && !parsed.name ? 'Consumer name was not detected.' : '', source.trim() && !routes.length ? 'No dispute or late-payment routes detected.' : ''].filter(Boolean);
  function switchRound(next: Round) { setRound(next); const first = library.find((item) => item.round === next)!; setPacketId(first.id); setDocId(first.docs[0].id); setOutputs([]); }
  ...
  async function uploadTemplate(file: File) { await storeFile(document.id, file); patchDoc({ file: file.name, fileType: file.type, extension: ext(file.name), size: file.size }); setStatus(`${document.name} saved. DOCX tags will render during generation.`); }
    ...
    for (const route of routes) {
      ...
      for (const doc of targetPacket.docs.filter((item) => item.file)) {
        ...
        if (doc.placeholders && isDocx(original.name)) {
          try { const blob = await renderDocxTemplate(original, values(route)); made.push({ id: `${route.bureau}-${doc.id}`, name, packet: targetPacket.name, document: doc.name, bureau: route.bureau, rendered: true, detail: 'DOCX · placeholders rendered', blob }); }
          catch { failed.push(doc.name); }
        } else made.push({ id: `${route.bureau}-${doc.id}`, name, packet: targetPacket.name, document: doc.name, bureau: route.bureau, rendered: false, detail: `${doc.extension || ext(original.name)} · exact copy`, blob: original });
      }
    }
    setOutputs(made); setPanel('Outputs'); setLoading(false); setStatus(failed.length ? `${made.length} files created; failed to render tags in ${failed.join(', ')}.` : `${made.length} files created. DOCX tags rendered; other documents kept unchanged.`);
  }
  ...
  function templateScreen() { return <><section className="panel" style={{ marginBottom: 18 }}><div className="panel-heading"><div><h2>Choose template round</h2><p>Every round keeps its own dispute and late-payment packet.</p></div><Pill tone="accent">{round}</Pill></div>{roundTabs()}</section><div className="content-grid"><section className="panel"><div className="panel-heading"><div><h2>{round} packets</h2><p>Upload original templates by document type.</p></div><Pill tone={savedCount ? 'success' : 'neutral'}>{savedCount}/{documents.length} saved</Pill></div><div className="packet-picker">{packets.map((item) => <button key={item.id} className={`packet ${item.id === packet.id ? 'selected' : ''}`} onClick={() => choosePacket(item)}><span>{item.name}</span><small>{item.description}</small><b>{item.docs.filter((doc) => doc.file).length}/{item.docs.length}</b></button>)}</div><div className="documents">{packet.docs.map((doc, i) => <button key={doc.id} className={`document ${doc.id === document.id ? 'selected' : ''}`} onClick={() => setDocId(doc.id)}><i>{i + 1}</i><span><strong>{doc.name}</strong><small>{doc.file || 'No file uploaded'}</small></span><Pill tone={doc.file ? 'success' : 'warning'}>{doc.file ? 'Saved' : 'Pending'}</Pill></button>)}</div></section><section className="panel editor-panel"><div className="panel-heading"><div><h2>{document.name}</h2><p>{packet.name}</p></div><Pill tone={document.file ? 'success' : 'warning'}>{document.file ? 'Saved' : 'Pending'}</Pill></div>{document.file ? <div className="saved-file"><strong>{document.file}</strong><span>{bytes(document.size)} · {document.extension}</span><p>{document.placeholders && isDocx(document.file) ? 'DOCX placeholder replacement enabled.' : 'Exact original copy on export.'}</p></div> : <div className="upload-empty"><p>No template uploaded.</p></div>}<label className="field-label">{document.file ? 'Replace template' : 'Upload template'}<input className="file-input" type="file" accept=".docx,.pdf,.png,.jpg,.jpeg" onChange={(e) => { const file = e.target.files?.[0]; if (file) void uploadTemplate(file); e.target.value = ''; }} /></label>{document.file && <button className="delete-button" onClick={() => void deleteTemplate()}>Delete saved file</button>}<label className="switch-row"><input type="checkbox" checked={document.placeholders} onChange={(e) => patchDoc({ placeholders: e.target.checked })} /><span>Replace approved placeholders in DOCX</span></label>{document.placeholders && <label className="field-label">Use these tags inside the DOCX<textarea className="code-area" value={document.fields} onChange={(e) => patchDoc({ fields: e.target.value })} /></label>}</section></div></>; }
  function sourceScreen() { return <div className="content-grid"><section className="panel"><div className="panel-heading"><div><h2>TXT source</h2><p>Import client data for routing.</p></div></div><label className="field-label">Upload .txt<input className="file-input" type="file" accept=".txt" onChange={(e) => e.target.files?.[0] && loadSource(e.target.files[0])} /></label><textarea className="source-area" value={source} onChange={(e) => setSource(e.target.value)} placeholder="Paste TXT source here…" /></section><section className="panel"><div className="panel-heading"><div><h2>Select output round</h2><p>Data will populate documents saved under this round.</p></div><Pill tone="accent">{round}</Pill></div>{roundTabs()}<div className="panel-heading" style={{ marginTop: 20 }}><div><h2>Detected routes</h2><p>{parsed.name || 'No consumer loaded'}</p></div><Pill tone={routes.length ? 'success' : 'neutral'}>{routes.length} routes</Pill></div><div className="route-table">{bureaus.map((bureau) => <div className="bureau" key={bureau}><strong>{bureau}</strong><Pill tone={parsed.dispute[bureau].length ? 'success' : 'neutral'}>Dispute</Pill><Pill tone={parsed.late[bureau].length ? 'success' : 'neutral'}>Late Payment</Pill></div>)}</div><button className="action-button" onClick={() => setPanel('Generate')}>Continue with {round}</button></section></div>; }
  function generateScreen() { return <div className="content-grid"><section className="panel"><div className="panel-heading"><div><h2>Generate {round}</h2><p>One output set per detected bureau/type.</p></div><Pill tone={routes.length ? 'accent' : 'neutral'}>{routes.length} routes</Pill></div>{routes.length ? <div className="route-list">{routes.map((route) => <div className="route-card" key={`${route.bureau}-${route.kind}`}><strong>{route.bureau}</strong><span>{route.kind.replace('_', ' ')}</span><small>{packets.find((item) => item.kind === route.kind)?.name}</small></div>)}</div> : <Empty title="No routes detected" text="Upload TXT source data first." />}<button className="action-button" disabled={loading} onClick={() => void generate()}>{loading ? 'Rendering documents…' : 'Generate documents'}</button></section><section className="panel"><div className="panel-heading"><div><h2>Rendering rules</h2><p>Free DOCX renderer is active.</p></div></div><div className="alert success">DOCX templates: approved tags are replaced and output remains .docx.</div><div className="alert success">PDF/image/attachment files: exported as exact copies with the same file type.</div>{blockers.map((text) => <div className="alert error" key={text}>{text}</div>)}{missing.length > 0 && <div className="pending-list"><p>{missing.length} documents are pending; they do not block setup.</p>{missing.slice(0, 5).map((doc) => <span key={doc.id}>{doc.name}</span>)}</div>}</section></div>; }
  function outputsScreen() { return <section className="panel outputs"><div className="panel-heading"><div><h2>{round} output files</h2><p>Rendered letters and unchanged supporting documents.</p></div><Pill tone={outputs.length ? 'success' : 'neutral'}>{outputs.length} files</Pill></div>{outputs.length ? <div className="output-list">{outputs.map((file) => <article className="output" key={file.id}><div><Pill tone={file.rendered ? 'success' : 'accent'}>{file.rendered ? 'Rendered DOCX' : 'Exact copy'}</Pill><h3>{file.name}</h3><p>{file.bureau} · {file.document} · {file.detail}</p></div><button className="secondary-button" onClick={() => save(file.name, file.blob)}>Download</button></article>)}</div> : <Empty title="No output files" text="Select a round, upload source data, then generate." />}</section>; }
  function settingsScreen() { return <section className="panel settings"><div className="panel-heading"><div><h2>Preferences</h2><p>Document generation controls.</p></div></div><label className="setting"><input type="checkbox" checked={strict} onChange={(e) => setStrict(e.target.checked)} /><span><strong>Strict template validation</strong><small>Require every document in the selected round before generating.</small></span></label><div className="info-card"><strong>DOCX rendering module</strong><p>DOCX placeholder replacement uses the free renderer installed in this project. All non-DOCX supporting files are retained as exact copies.</p></div></section>; }
