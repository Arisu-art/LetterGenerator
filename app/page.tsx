...
export default function Page() {
  ...
  const [packet, setPacket] = useState<PacketAssets>({ supporting: [], legalPdf: null });
  ...
  useEffect(() => { if (ready) setPacket(loadPacketAssets(round)); }, [ready, round]);
  ...
  function replaceSource(value: string) { setSource(value); setSourceOriginal(''); setSourceNormalized(false); resetPackage(); }
  function automaticallyNormalizeSource(value: string, method: 'uploaded' | 'pasted') {
    ...
    resetPackage();
    setStatus(`${method === 'uploaded' ? 'Uploaded' : 'Pasted'} TXT automatically standardized. Review detected records, or restore original data at any time.`);
  }
  function normalizeSource() {
    ...
    setSourceNormalized(true);
    resetPackage();
    setStatus('Current edits standardized. Supplemental or unrecognized text remains protected and is not inserted unless mapped by a document.');
  }
  function restoreOriginal() { if (!sourceOriginal) return; setSource(sourceOriginal); setSourceOriginal(''); setSourceNormalized(false); resetPackage(); setStatus('Original TXT source restored.'); }
  ...
  function manifest(files: Output[], warnings: string[], date: string, notes: string[]) {
    ...
    return [
      ...
      '',
      'Assembly Rules:',
      `Supporting Evidence: ${packet.supporting.length ? `${packet.supporting.length} page(s) appended inside each generated DOCX.` : 'Not included.'}`,
      `FCRA Legal Exhibit: ${packet.legalPdf ? 'Packaged beside each generated Dispute DOCX only.' : 'Not included.'}`,
      '',
      'Output Decisions:',
  ...
  async function buildZip(files: Output[], warnings: string[], date: string, notes: string[]) {
    ...
    if (packet.legalPdf) {
      const legalPdf = await loadPacketFile(round, packet.legalPdf.id);
  ...
  async function generate() {
    ...
    try { supportingPages = await getSupportingPages(round); }
  ...
  function templatesView() {
    return <div className="templates-workspace">
      <div className="content-grid">
        <section className="panel"><div className="panel-heading"><div><h2>Editable letter references</h2><p>Upload one finished DOCX reference per letter type and per round.</p></div><Pill tone="accent">{round}</Pill></div>{roundTabs()}<div className="documents">{roundSlots.map((slot, index) => <button key={slot.id} className={`document ${slot.id === selected.id ? 'selected' : ''}`} onClick={() => setSelectedId(slot.id)}><i>{index + 1}</i><span><strong>{slot.name}</strong><small>{slot.file || 'Finished output reference DOCX required'}</small></span><Pill tone={slot.file ? 'success' : 'warning'}>{slot.file ? 'Saved' : 'Needed'}</Pill></button>)}</div></section>
        <section className="panel editor-panel"><div className="panel-heading"><div><h2>{selected.name}</h2><p>Editable DOCX reference for generated output</p></div><Pill tone={selected.file ? 'success' : 'warning'}>{selected.file ? 'Saved' : 'Needed'}</Pill></div>{selected.file ? <div className="saved-file"><strong>{selected.file}</strong><span>{bytes(selected.size)} · DOCX</span><p>Generated output uses this document&apos;s content regions and format.</p></div> : <div className="upload-empty"><p>Upload a completed DOCX showing how this letter type should look.</p></div>}<label className="field-label">Upload or replace DOCX reference<input className="file-input" type="file" accept=".docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadSlot(file); event.target.value = ''; }} /></label>{selected.file && <button className="delete-button" onClick={() => void deleteSlot()}>Delete saved file</button>}<div className="info-card"><strong>{selected.type === 'DISPUTE' ? 'Dispute letter reference' : 'Late Payment letter reference'}</strong><p>{selected.type === 'DISPUTE' ? 'Include client, date, bureau, account/item area, inquiry area and signature sections.' : 'Include client, date, bureau, creditor/account and signature sections.'}</p></div></section>
      </div>
      <PacketAssemblySetup round={round} onChanged={(next) => { setPacket(next); resetPackage(); }} onMessage={setStatus} />
    </div>;
  }
  function sourceView() {
    return <div className="source-workspace"><section className="panel source-input-panel"><div className="panel-heading"><div><h2>Source TXT</h2><p>Upload or paste client records. Incoming source is automatically standardized without deleting the original data.</p></div>{source && <Pill tone={parseWarnings.length ? 'warning' : 'success'}>{parseWarnings.length ? `${parseWarnings.length} review` : 'Validated'}</Pill>}</div><div className="source-actions"><label className="field-label">Upload TXT file<input className="file-input" type="file" accept=".txt" onChange={async (event) => { const file = event.target.files?.[0]; if (file) automaticallyNormalizeSource(await file.text(), 'uploaded'); event.target.value = ''; }} /></label>{!source.trim() && <button className="secondary-button" onClick={() => { replaceSource(recommendedSourceFormat); setStatus('Standard TXT format inserted. Replace example values with client data.'); }}>Use standard format</button>}</div>{source.trim() && <div className="normalization-actions">{!sourceNormalized && <button className="normalize-source" onClick={normalizeSource}>Standardize current edits</button>}{sourceOriginal && <button onClick={restoreOriginal}>Restore original data</button>}</div>}{sourceNormalized && <div className="normalized-source-banner"><div><strong>Automatically standardized review copy active</strong><p>The uploaded or pasted original is preserved for restoration. Supplemental data is kept below standard records and is not inserted into letters unless mapped.</p></div></div>}<textarea className="source-area" value={source} onPaste={handleSourcePaste} onChange={(event) => { setSource(event.target.value); setSourceNormalized(false); resetPackage(); }} placeholder="Paste TXT source here, or select Use standard format to begin…" /></section>{sourceReady ? <section className="panel source-results-panel"><div className="panel-heading"><div><h2>Detected letters</h2><p>Confirm the bureau and letter type before continuing.</p></div><Pill tone="accent">{routes.length} output{routes.length === 1 ? '' : 's'}</Pill></div><div className="detection-table">{bureaus.map((bureau) => { const dispute = parsed.dispute[bureau].length; const inquiry = parsed.inquiry[bureau].length; const late = parsed.late[bureau].length; return <div className="detection-row" key={bureau}><strong>{bureau}</strong><span>{dispute || inquiry ? `${dispute} dispute · ${inquiry} inquiry` : 'No dispute data'}</span><span>{late ? `${late} late payment` : 'No late payment'}</span></div>; })}</div>{(parsed.phone || parsed.email || parsed.preserved.length > 0) && <section className="source-protection"><div className="source-protection-header"><div><strong>Preserved supplemental data</strong><p>Retained for supporting documents or future mappings. Not inserted into dispute letters automatically.</p></div><Pill tone="neutral">Protected</Pill></div><div className="protected-fields">{parsed.phone && <span className="reserved">PHONE: {parsed.phone}</span>}{parsed.email && <span className="reserved">EMAIL: {parsed.email}</span>}{parsed.preserved.filter((item) => !/^(PHONE|TELEPHONE|MOBILE|EMAIL|E-?MAIL)\s*:/i.test(item.text)).slice(0, 3).map((item) => <span key={`${item.line}-${item.text}`}>Line {item.line}: {item.text}</span>)}</div><p className="source-rule">Extra text is never silently removed. It is separated from generated-letter data until a matching document rule is created.</p></section>}{parseWarnings.length > 0 && <section className="source-review"><strong>Review before generating</strong>{parseWarnings.slice(0, 4).map((warning, index) => <p key={`${warning.message}-${index}`}>{warning.line ? `Line ${warning.line}: ` : ''}{warning.message}</p>)}</section>}<button className="action-button" disabled={!generationReady} onClick={() => setPanel('Generate')}>Review {routes.length} detected letter{routes.length === 1 ? '' : 's'}</button></section> : <section className="panel source-guide"><h2>TXT source standard</h2><p>Paste or upload source data and the system will standardize a review copy automatically. Supplemental fields remain protected until a document maps them.</p><div className="guide-steps"><div><strong>1</strong><span>Upload or paste the original client source</span></div><div><strong>2</strong><span>Review the standardized copy and preserved fields</span></div><div><strong>3</strong><span>Confirm bureau letters before generation</span></div></div><p className="guide-note">Hard inquiries should use: <strong>COMPANY - MM/DD/YYYY</strong></p></section>}</div>;
  }
  function assemblyChips(route: LetterRoute) {
    return <div className="assembly-chips"><span className="assembly-chip">Editable letter</span>{packet.supporting.length > 0 && <span className="assembly-chip shared">{packet.supporting.length} supporting page{packet.supporting.length === 1 ? '' : 's'}</span>}{route.type === 'DISPUTE' && packet.legalPdf && <span className="assembly-chip legal">FCRA PDF companion</span>}</div>;
  }
  function generateView() {
    ...
    return <div className="generation-workspace"><section className="panel generation-overview"><div><p className="eyebrow">Packet preparation</p><h2>Prepare {round} delivery</h2><p>Verify every letter and attachment rule before creating the final ZIP package.</p></div><div className="generation-summary"><div><strong>{routes.length}</strong><span>DOCX letters</span></div><div><strong>{packet.supporting.length}</strong><span>Shared pages</span></div><div><strong>{hasDispute && packet.legalPdf ? 'Yes' : 'No'}</strong><span>FCRA PDF</span></div></div></section><div className="generation-layout"><section className="panel route-production"><div className="panel-heading"><div><h2>Packet output plan</h2><p>Supporting evidence is appended in the DOCX. FCRA is packaged beside Dispute DOCX outputs only.</p></div><Pill tone="accent">{routes.length} ready</Pill></div><div className="production-rows">{activeBureaus.map((bureau) => <section className="bureau-output" key={bureau}><header><strong>{bureau}</strong><span>{routes.filter((route) => route.bureau === bureau).length} letter{routes.filter((route) => route.bureau === bureau).length === 1 ? '' : 's'}</span></header>{routes.filter((route) => route.bureau === bureau).map((route) => <div className="production-row" key={`${route.type}-${route.bureau}`}><div><strong>{label[route.type]}</strong><small>{route.reason}</small>{assemblyChips(route)}</div><Pill tone="success">Create</Pill></div>)}</section>)}</div></section><aside className="panel execution-panel"><div className="panel-heading"><div><h2>Ready to generate</h2><p>Final pre-flight confirmation.</p></div></div><div className="readiness-checks"><div className={sourceReady ? 'checked' : ''}><strong>Source verified</strong><span>{parsed.name || 'Client missing'}</span></div><div className={!missingTypes.length ? 'checked' : 'blocked'}><strong>Letter references</strong><span>{missingTypes.length ? `Missing ${missingTypes.map((type) => label[type]).join(', ')}` : 'All required DOCX layouts uploaded'}</span></div><div className={packet.supporting.length ? 'checked' : 'review'}><strong>Supporting Documents</strong><span>{packet.supporting.length ? `${packet.supporting.length} page(s) appended to each letter` : 'No evidence pages uploaded'}</span></div>{hasDispute && <div className={packet.legalPdf ? 'checked' : 'review'}><strong>FCRA Legal Exhibit</strong><span>{packet.legalPdf ? 'Packaged with each dispute output' : 'No dispute-only PDF uploaded'}</span></div>}</div>{blockers.map((item) => <div className="alert error" key={item}>{item}</div>)}<button className="action-button generate-primary" disabled={loading || (strict && missing.length > 0)} onClick={() => void generate()}>{loading ? 'Assembling document packets…' : `Generate ${routes.length} Document Packet${routes.length === 1 ? '' : 's'}`}</button><p className="generation-note">After generation, review editable DOCX letters before downloading the final ZIP package.</p></aside></div></div>;
  }
