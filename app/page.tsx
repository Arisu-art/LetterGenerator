...
const category: Record<Kind, { label: string; folder: string }> = {
  DISPUTE: { label: 'Dispute Letter', folder: 'Dispute Letters' },
...
function seedLibrary(): Packet[] { return rounds.flatMap((round, index) => {
  ...
  return [
    { id: `${index + 1}-dispute`, round, kind: 'DISPUTE', name: `${round} Dispute Letter`, description: 'Generate only when dispute account data exists for a bureau.', docs: disputeDocs(`${prefix}dispute`) },
...
function outputRoutes(parsed: Parsed): Route[] { return bureaus.flatMap((bureau) => {
  ...
  const routes: Route[] = [];
  if (disputes.length) routes.push({ bureau, kind: 'DISPUTE', records: disputes, disputeRecords: disputes, inquiryRecords: [], reason: 'Valid dispute account data detected for this bureau.' });
  if (late.length) routes.push({ bureau, kind: 'LATE_PAYMENT', records: late, disputeRecords: [], inquiryRecords: [], reason: 'Valid late-payment data detected for this bureau.' });
  return routes;
}); }
...
export default function Page() {
  ...
  const blockers = [!source.trim() ? 'Upload or paste TXT source data.' : '', source.trim() && !parsed.name ? 'Consumer name was not detected.' : '', source.trim() && !routes.length ? 'No valid dispute or late-payment data was detected.' : ''].filter(Boolean);
  ...
  async function generate() {
    ...
    setStatus(`${made.length} files created inside one ZIP. Only dispute and late-payment letters supported by valid bureau data were created.`);
  }
